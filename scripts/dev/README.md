...
仅保留**可复用**的工具（下表 10 项 = 1 个浏览器探针入口 + 1 个快照夹具生成器 + 1 个破坏性验收脚本 + 6 个纯 JS 单测 + 本说明）。
历史一次性探针（p00/p02/p0x 视觉批、u11~u76 用户实测核查、r1 视觉重设计截图共 72 个）已于 2026-09-13 随仓库瘦身**整体删除**：它们引用已删除的 docs 证据目录、且多数已不可运行，仅作历史留痕，价值低于维护成本。需要考古时从 git 历史取（`git log --diff-filter=D -- scripts/dev/archive/`）。
...
- 新增探针/工具：可复用的放本目录并在上表登记；**一次性核查脚本用完即删**（2026-09-13 起不再设 `archive/` 目录——历史脚本一律不留），保持本目录 ≤ 10 个文件。
