"""命令行入口模块（C3 拆分自 main.py；D1 增加命令行参数与非交互模式）。

职责：组合各模块完成主控制流——解析命令行参数（可选 target 与 --top），按
...
from utils import APP_NAME, _exit_with_error, _fatal, human_size, log
...
def _parse_args(argv):
    """argparse 参数定义：可选位置参数 target + 可选 --top N（默认 10，校验 1-200）。"""
...
def main(argv=None):
    ...
    - 未提供 target（含只给 --top）→ 交互模式 _run_interactive，行为与拆分前逐字一致；
    ...
    return _run_headless(args.target, args.top)
...
def _run_headless(raw_target, top_n):
