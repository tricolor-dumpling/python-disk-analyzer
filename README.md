...
- **状态栏**（阶段E·E-6，D4 裁定落地）：左=「v2.0.0」版本号；右=「已选 N 项」（列表多选时出现，清零隐藏）。固定说明文案（数据保存路径 / Everything 驱动 / 本地监听地址）已删除——信息迁入设置弹窗（`setting-data-dir`）与健康徽章 popover（数据目录 / 驱动状态 / 监听地址 127.0.0.1），信息不丢；状态栏高度预算 32px 不变（F22）。
...
- 顶栏按钮：亮 ↔ 暗显式切换（点击即生效；圆形扩散转场 450ms 自点击处展开；系统减弱动效时直切）；顶栏切换会写入显式偏好并退出「跟随系统」。
- 设置弹窗：主题分段单选（亮 / 暗 / 跟随系统），选择即生效（与「保存设置」按钮解耦）；**扩散中心=真实鼠标坐标**（阶段E·E-4：pointerdown 记录 `clientX/clientY`，键盘/触摸无坐标时回退选项中心）。
- **VT 连点防护（阶段E·E-3）**：主题扩散使用 View Transitions + 圆形 clip-path；快速连点/四角/四边任意位置触发均无整页一次性铺满帧（新转场启动前对旧转场显式 `skipTransition()`，全局 only-one-VT 队列；扩散圆半径 +16px 冗余防滚动条抖动；动画结束移除内联 clip-path 不残留）。
...
- **关系目录（阶段C·C-7）**：第 4 视图「关系」= 父子层级树（D3 裁定）：树根=当前浏览数据，目录行可展开（`▸` caret + 文件夹图标）、文件为叶子；**懒展开**= 单击目录行走 `browsePath` 下钻（复用既有加载，1 次/节点按需请求）；>200 子项虚拟化（行高固定 36px，同列表口径）；键盘可达（`↑↓` 移动、`→`/`Enter` 下钻展开、`←` 返回上级、`Home`/`End` 首尾）；与矩形图/排行/表格互斥终态（防视图残留，同 C-1 断言面）。
- **行高单一档（阶段P3·D3-6，原「密度差异」已删除）**：列表与关系树行高**固定 36px**，不再提供「紧凑列表/舒适列表」密度开关——原 compact 档会 `display:none` 掉占比条并把占比列压成 `width:0`（信息损失），收益仅 1.38×，且 cozy 的虚拟滚动窗口 DOM 行数更少。完整图标 + 占比条为常驻信息。
- **多选与导出**：复选框 + `Shift` 范围选；页脚 sticky「共 N 项 · 已选 N 项 · [定位所选] [导出所选 CSV]」；CSV 由前端生成（UTF-8 BOM，Excel 可直接打开，列表头 名称/路径/类型/大小(字节)/大小(可读)）。
- **虚拟滚动**：超过 200 行启用，固定行高 36px 零漂移，滚动窗口重渲染不重放入场动画。
- **视图工具栏**：`−/+` 调整「其他」合并阈值（仅矩形图视图；默认 24，步长 10，范围 1–200）；「全屏」= 视图区铺满（`Esc` 退出）。**（阶段P3·D3-6：「紧凑列表/舒适列表」密度开关已整体删除——DOM/状态/CSS/文档/探针五处入口全部收口，无隐藏开关、无 `.compact-list` 类、无 `view.density` 字段。）**
...
**右栏卡**（阶段P3·D3-2 起**区间自适应** `clamp(280px,22vw,380px)`：1024→280 / 1366→300.5 / 1440→316.8 / 1920→380px；<900px 宽仍为声明例外整宽）：

- **存储概览卡**：环形图（已使用之环比——接口无真实容量字段，故不伪装成磁盘容量百分比）+ 盘符 chips（点击只切换环形图数据、不切换浏览目录；「浏览此盘」是唯一跳转浏览入口）+ 图例两行（本盘已用/全部盘累计）；全量扫描中自动跟随当前盘，用户手选后本扫描期内锁定；**扫描中不确定弧 1.2s 循环旋转**（阶段E·E-1：`@keyframes donut-spin` 从 -90°→270° 整 360° 循环 + `transform-box:view-box` 显式声明，无循环回跳；中心数字 count-up 单调渐增不归零重播）；卡头含导出 CSV / 导出 JSON（阶段B·B-12 移入结果区，可用性=完成/中止部分根，扫描中禁用）。**卡头与图例排版（阶段P3·D3-4/D3-5）**：标题列 `word-break:keep-all`（CJK 标题按整词换行，杜绝「存储概 / 览」竖排）、动作组 `flex:none` + 卡头 `flex-wrap:wrap`（一行放不下时整组换行，**不挤压标题**）；`.storage-row` 改 grid（`auto minmax(0,1fr)`）、图例标签与数值显式「换行」（`white-space:normal` + `overflow-wrap:anywhere`，原先只有裸 `nowrap` 而被右栏 `overflow-x:hidden` 静默硬切）。
...
- **自动扫描（阶段D·D-1，方案 A 自动模式）**：打开页面且 Everything 已就绪、无当日快照会话、本页会话未自动扫过 → **自动发起一次全量扫描**（`sessionStorage.pds_auto_started_v1` 一次性保护键——刷新不重复、扫描中刷新恢复运行态不重发、Everything 未就绪不发起且就绪后不补触发、启动失败不写键下次刷新可自动重试）；冷启动自动扫描期间启动浏览延后到扫描完成（走全量索引，零 409、零重复 SDK 直扫）。
- **空闲**：「开始全量扫描」按钮——自动模式下空闲且有结果变**「重新扫描」**、失败变**「重试扫描」**（顶栏同步）；空闲点击进入「启动中…」提交态，提交结果经状态广播收敛。
- **排队中**（阶段B·B-8）：SDK 锁被浏览/对比/上一轮尾段占用时，状态行显示「等待扫描引擎空闲…（扫描任务已提交，正在排队）」+ 盘符 chips 保持待办灰（不显示误导性「0/2 扫描中」）；前端轮询降频 2s。
- **扫描中**：进度条斜纹流光 + 头部亮点；状态行整合「总进度 % · 已完成 x/y 盘 · 当前 C:\」+ 已用时 + 预计剩余 ~T（估算，基于根间均速，标注「估算」不闪跳——变化 >10% 才更新，阶段D·D-3b）；盘符 chips（已完成 ✓ / 进行中脉冲 / 待办灰）；按钮变红描边「停止」；扫描中可继续浏览已完成盘（当前盘返回进行中提示）。
- **完成**：进度条绿光扫过 + 条尾对勾描边 + 16 粒粒子庆祝（仅主页可见时播）+ **自动保存（P1·D1-1 后端归口）**：全量结果就绪由**后端**读取 `config.auto_save`（缺键=ON）触发一次自动保存，前端只展示结果（不再自行发起、杜绝双保存）；扫描卡呈现三态——**已自动保存**（notice-success）/ **已跳过自动保存 + 精确原因**（notice-warn，如「该根今天已自动保存过」；`保存快照` 保持可用 + 「仍要保存（强制）」进入，走 `auto:false` 显式保存不受谓词限制）/ **自动保存失败**（notice-error，失败原因可见，手动入口保留）。
...
- **1366×768 紧凑档**（`max-height:820px`）：右栏自动收紧——折叠次级说明文案（保留主文案与动作）、卡内 padding/间距收紧、右栏小按钮/通知块纵向收紧；**整页无内滚、无溢出**。阶段P3 实测（`p03_rail_layout_probe.mjs`，满数据桩态：三盘概览 + 长盘名 + 真实快照）：右栏内滚量 idle **0px**（P2 基线 138px）、done 4px（基线 228px）、scanning 58px（基线 484px，注入长路径/长 ETA 的最坏态）。「1366×768 右栏无内滚」的硬判据 = `u42:573-578` 的 P6 口径，p03 探针在 **idle 态**同口径自证（Δ ≤ 1px）。
- 1920×1080 常规档在「扫描完成 + 保存提示条」满态下右栏出现面板内滚 = 声明允许的说明项（仅紧凑档保证无内滚）。
- **右栏文本截断策略（阶段P3·D3-4）**：右栏内所有可能溢出的文本必须显式选择「换行」或「省略号」，禁止裸 `white-space:nowrap` + 无 overflow；`.btn` 自带 `nowrap`（按钮标签不换行是 UI 约定），故右栏按钮一律 `flex:none`（换行而不被压窄裁字），chips 允许词内换行且 `max-width:100%`（长盘名全文可见，`title` 保留完整路径）。
- **度量 token 归位（阶段P3·DEF-P3-01 修复）**：`--fs-*` / `--radius-*` / `--space-*` 由 `style.css` 迁入 `tokens.css` 的 `:root`。迁移同时修掉一个长期潜伏的缺陷——`style.css` 原文件头注释里写有 `（--fs-*/--radius-*/--space-*）`，其中 `*`+`/` 相邻会**提前闭合注释**，使其后的 `:root{…}` 被当作「非法选择器」整条丢弃：实测 `document.styleSheets` 中无 `:root` 规则、`--fs-xs` 计算值为空，全站 87 处 `var(--fs-*)` 与 36 处 `var(--radius-*)` 全部失效（字号回落 16px、圆角回落 0）。修好后右栏 1366×768 满态内滚量由 138px 降到 0px（同一支探针 A/B 实测）。
...
- **Web 版自动保存（P1·D1-1 后端归口）**：全量扫描结果就绪时由**后端**读取
  `config.auto_save`（缺键视为 ON，设置弹窗同步）触发一次自动保存，前端只负责展示
  结果；扫描卡呈现三态（已自动保存 / 已跳过+原因 / 失败+手动补救）；跳过原因
  精确透出（如 `fingerprint_unchanged`/`already_saved_today`/`day_budget_exceeded`），
  `保存快照` 保持可用 + 「仍要保存（强制）」进入（`POST /api/save {auto:false}`，
  显式保存不受四原子谓词限制）；服务重启后首次扫描、子页面（`#/compare`、`#/snapshots`）
  完成均会自动保存（后端归口天然覆盖，跨进程代次闸门已移除）；`DSA_NO_SNAPSHOT` /
  `snapshots.is_snapshot_disabled()` 时绝不自动落盘。快照管理页可「创建快照」与
  「撤销最近保存」。
...
### 深度选择、聚合与下钻（P4，问题 5 / D4-1…D4-6）

`/api/compare` 新增三个**可选**参数；缺省语义与 v2.0.0 逐字段一致（契约用例 +
`git show stage-p3:compare.py` 直调引擎逐字段比对证明，见
`docs/问题核查资料_20260908/p4/backcompat-postfix.txt`）：

| 参数 | 取值 | 缺省 | 语义 |
|---|---|---|---|
| `depth` | ≥1 整数 | 不传 = 叶子口径 | 键集合先折叠到「相对对比根的第 N 层」再产行——**聚合发生在排序/截断之前** |
| `drop_zero` | 布尔 | `false` | 排序与 Top-N 截断**之前**剔除 `delta == 0` 的行 |
| `order_by` | `delta` / `abs` | `delta` | Top-N 切片排序键；`abs` 时按 `|delta|` 降序（正负增量同榜） |

聚合口径（`compare._rollup`）：折叠目标键本身即为一行且**取其自身值**（目录值
已含全部后代），目标不是键（层级缺口）时取组内顶层成员之和——真实扫描数据
「每目录/文件皆有行、父含子」（scan.py 自底向上汇总）下，
`Σ(聚合行 delta) == delta_total` 精确成立；被更深目标覆盖的更浅目标不入行集，
根行不进深度行集（根合计由摘要卡 `delta_total` 承载）。

Web 对比工作台（`#/compare`）对应功能：页头**深度选择器**（叶子（默认）/ 1..5 层）、
**隐藏零变化**开关（默认开 → 请求带 `drop_zero=true&order_by=abs`）、**行点击页内下钻**
（以该目录为新根重新对比，后端把基线行按新根收窄）+ **面包屑**逐级返回；结果缓存键
并入 `depth` 与本页开关口径（口径变更必重发，跨页共享缓存仍按既有「落地渲染不重发」语义）。

摘要口径（D4-5）：响应 additive 返回 `rows_total` / `zero_count` / `zero_total` /
`max_growth` / `max_release` / `depth`——前三与 `delta_total` 同为**全量聚合行**口径
（未按 100 条切片），消除「摘要与总变化自相矛盾」。`/api/compare/status` 三个分支
均回显该任务的 `depth` / `drop_zero` / `order_by`。

CLI / TUI 调用方**零改动**：`top_growth` 默认仍按有符号 delta 降序、`diff_from_current`
默认行集合语义不变（cli.py:518、tui.py:593/608 回归红线）。

### 术语：对比基准（历史快照）与当前磁盘状态（P5，问题 7 / D5-1、D5-2、D5-5）

对比的两侧在界面上**如实表述**，不再用「基线 / 目标」这类必须靠 `title=` 悬停才能
看懂的词：

| 侧 | 界面用词 | 数据来源（实测口径） |
|---|---|---|
| 历史侧 | **对比基准（历史快照）** | 用户选定的某一份 `.snap.gz` 快照文件 |
| 现状侧 | **当前磁盘状态（实时）** | `fullscan.result(root)` 内存结果，或 Everything SDK 直扫——**不是快照文件** |

> 为什么要改：P5 之前界面把现状侧标成「目标 = 同盘符最新快照」，而代码注释
> （原 `compare.js:11-14`）自己就写着「『当前』侧并非快照文件，『目标=同盘符最新快照』
> 只是标识性文案」——把一个"当前磁盘状态"伪装成"目标快照"，这正是用户反馈
> 「基线/目标意义不明」的直接成因。**P5 只改表达，计算口径一字未动**
> （`compare.py` 零改动，由 `docs/问题核查资料_20260908/p4/_verify_backcompat.py`
> 复跑 `BACKCOMPAT-VERDICT: PASS` 自证）。

对比页（`#/compare`，标题「空间对比」）页头：**对比基准下拉**（每项「时间 · 盘符 ·
自动/手动」，最近一份标注「最近一份」）+ **当前磁盘状态**只读文本行（`当前：D: ·
数据时间 …`，非表单控件）+ 深度选择器 + 隐藏零变化开关 + 开始对比。
原只读「目标」输入框与 `#baseline-suggest` datalist 已删除。

### 首次启动选盘与默认根（P5，问题 7 / D5-3、D5-4）

- 新增 additive 接口 **`GET /api/roots`** → `{ok, roots:[{root,label,ready}], count,
  drives_source}`；枚举**只读复用** `fullscan._enumerate_roots()`（Win32
  `GetLogicalDrives` 优先，失败/非 Windows 回退 A–Z 探测），`ready=false` 表示
  未就绪/无介质（**保留在列表里并标注**，不静默丢弃）。
- 首启引导弹层新增第 2 步「**选择要分析的盘**」：选项来自上面的接口（不再硬编码
  `C:\ D:\ E:\ F:\`），未就绪盘灰置；选中后写入设置 `last_roots` 并立即成为工作台
  浏览根。该步骤**可跳过**（跳过 = 维持既有默认逻辑，不阻塞首开），跳过态可点
  「重新选择盘」返回。
- **默认根去魔法值**：统一为「上次浏览位置 → 后端枚举盘的**第一个可用盘** →
...
- **已知限制（发布期登记，不擅自实现）**：
  - ~~**P-2 浏览历史下拉**（时钟图标形态）未落地~~ → **阶段 G 已落地（G-3）**：
    时钟按钮 → 下拉面板（复用 getBrowseHistory 最近 8 倒序，命令面板同款分组样式，
    键盘 ↑↓/Enter/Esc 可达、外部点击关闭、<2 条隐藏），功能与形态同时收口；
  - ~~**P-3 面包屑 >6 层折叠**未落地~~ → **阶段 G 已落地（G-4）**：>6 层折叠为
    首段 + 「…」 + 末两段，「…」点击展开全层（当次渲染瞬态）、键盘可聚焦、
    各层 data-path 回跳与既有语义一致；
  - ~~**P-4 趋势卡 sparkline 数据源**未落地~~ → **阶段 G 已落地（G-2）**：
    `/api/snapshots` 每会话 additive `total_by_root: {root: bytes}`（从快照文件派生，
    与差值卡 total_current 同口径）+ 前端趋势卡 sparkline 启用（多会话 ≥2 点画折线，
    描线 800ms/reduced 直显，终点脉冲仅 opacity）；
  - ~~**P-5 C 盘口径标注未落地**~~ → **阶段 G 已落地（G-1）**：D5 定稿「逻辑尺寸，
    未计硬链接重叠」已标注于概览卡图例行尾、chips title、环形 aria-label 与
    设置弹窗数据目录区（文案与 D5 裁定原文逐字一致）；
  - ~~**P-9 cli.py 非法转义**~~ → **阶段 G 已落地（G-5，代码卫生批次）**：`cli.py`
    2 处文档字符串 `exports\\` 非法转义改原始字符串 r"""（零功能改动）；验收
    `py_compile -W error::SyntaxWarning` 零告警（3.11.8 改前改后均 exit=0 如实记录，
    3.14 面构造性零告警——本机无 3.14 环境如实注明）；
  - ~~**P13 快照锁竞态**挂账不变~~ → **阶段 G 已修复（G-6，用户显式授权）**：
    P13-FIX 方案①（`_is_stale` 读 PID 改 ctypes CreateFileW FILE_SHARE_DELETE +
    `_release_lock` unlink 失败重试 3 次）；专项 50 轮 **0/50 失败**（修复前
    21/50），不再依赖「复现允许复跑」纪律；
  - ~~**2-2 视图残留**：七轮（A–G）未复现~~ → **阶段 P2 已修复（2026-09-10）**：
    历史七轮「未复现」经 P2 证明为**断言口径盲区**而非现象不存在——旧探针只断言
    700–800ms 后的终态，且 `u66` 连点序列结尾是 `"table","table"` 幂等点击
    （拍关键帧前已把终态拍平）。改为帧级断言后：同一支探针在 `p1-autosave@750176a`
    代码上复现 **128/488** 违规帧（6 组序列），修复后 **0/486**。关闭依据见下节 D2-4；
  - **真机 Everything 分钟级 busy（G8）**：SDK 直扫分钟级为环境常态，真机验收按
    「请求发起态」口径记录（compare 无缓存走 202 异步 + 前端 30s 超时已兜底）。

## 阶段 P2（工作台视图切换残留 · 问题 2，2026-09-10）

- **现象**：切到排行/关系/表格后矩形图整幅"闪透"并淡出 ≈120ms（用户问题 2）。
- **根因（四条）**：R1 `crossfadeView` 把方向判成「矩形图 vs 表格」二元 → **非矩形图→非矩形图**的
  切换也走交叉淡化（`removeAttribute("hidden")` + `opacity=1`）；R2 两容器同为 `position:absolute;inset:0`
  且无 z-index，`#treemap-wrap` 靠后 → 叠加时压在上层；R3 JS 已加 `.v-crossfade` 类但 CSS 中
  **不存在该规则**；R4 `treemap.pause()` 只 `cancelAnim()`（不重绘终帧、隐藏态仍响应 hover 重绘），
  且列表 hover→矩形图联动的委托缺可见性守卫。
- **修复**：①`treemapInvolved = prev === "treemap" || mode === "treemap"`，仅矩形图参与时才交叉淡化
  ②补 `.v-crossfade-in`(z-index:3) / `.v-crossfade-out`(z-index:2) 与 `pointer-events:none`（层序
  低于 `#browse-loading` 的 z-index:5，骨架屏仍覆盖视区）③`setPaused()` 真暂停 + `highlightKey`/指针链守卫
  ④列表 hover 联动前判 `#treemap-wrap` 可见性。
- **帧级前后对照（同一支探针、同一判据）**：`node scripts/dev/p02_view_frame_probe.mjs` →
  修复前 **128/488** 违规帧（6 组序列；30ms 连点 24）→ 修复后 **0/486**、闪烁重现 0、
  hover 隐藏期绘制调用增量 0。
- **证据**：`docs/问题核查资料_20260908/p2/`（`manifest.json`：A 类 rAF 逐帧 / B 类 CDP screencast
  像素帧 + 混合比例 / C 类三视口截图；11 张图片全部逐张判读）。

### D2-4 关闭口径：视图切换残留只能以**帧级断言**关闭

> 本节是问题 2 的关闭依据定义，对后续同类问题（视图/浮层残留）同样适用。

1. **唯一可接受的关闭依据 = 帧级（rAF 逐帧）断言**，且必须在**动画窗口内**成立。
   终态断言（切换后 500–800ms 的 `display`/`visibleCanvas` 采样）**不构成关闭依据**——
   本问题的历史七轮「未复现」正是终态口径的盲区。
2. **计账窗口**必须显式定义：[首次点击时刻, 末次点击 + 单步时长 + 200ms 收尾]；
   窗口之外的帧不计违规，但保留在产物中供核对。要求每组序列窗口内 ≥60 帧（≈1s @16.7ms）。
3. **违规判据（三项，缺一不可）**：目标视图 ≠ 矩形图时，`#treemap-wrap` 的
   ① `hidden` 属性、② computed `display`、③ 视区中心命中测试，三者都必须指向"不可见/不可命中"。
   - **不得以 computed/inline `opacity` 作判据**：`display:none` 容器上的 opacity 残值既不产生
     可见像素也不影响命中测试；以 opacity 判定会在连点序列产生假阳性（P2 实测 15 帧）。
   - **合法例外**：矩形图**参与**的切换（矩形图↔列表）其出向交叉淡化（`.v-crossfade-out`）
     期间矩形图必然在绘制 —— 这是 D2-1 要求保留的设计行为，不计违规；但该淡化必须在
     `--dur-1`(120ms) 内收束，**超过 250ms 仍在绘制即判违规（停滞）**。
4. **连点变体必须覆盖**（30ms 间隔 ≥10 次，页内定时器派发以免 Node↔浏览器往返把点击时刻记晚）：
   除违规帧外还须断言「闪烁式重现事件 = 0」，其中重现事件 = 窗口内矩形图由隐藏→可见的跃迁
   （**目标为矩形图时不计**——那是合法的入向淡化）。
5. **探针有效性自证（红线 A）**：新增/修改的帧级探针必须先在**修复前代码**上跑出违规帧，
   再在修复后跑出 0 违规。修复前也抓不到 → 探针口径被放宽，工具链不合格，不得据此宣布修复成功。
6. **像素证据（B 类）**：中间态一律走 CDP `Page.startScreencast`，**禁用 `page.screenshot()`
   验收 120ms 级动画**（≈130ms/张）。帧分类须先自证阈值可分（参照帧自身的 JPEG 化样本
   应落回本类），并要求混合比例随时间为非降；**同一份产物内同哈希的重复帧不得当证据**。

## 阶段 G（发布后挂账裁决落地与代码卫生批次，2026-09-06）
- **授权**：用户裁定 G-1 补齐 / G-2 补数据源+前端启用 / G-3 形态收口 / G-4 折叠实现 /
  G-6 P13-FIX（显式授权方案①）；本提示词直接授权 G-5 代码卫生批次。
- **落地项**（每项独立变更集，见交付报告）：
  - G-1 P-5 口径标注（storage.js/settings.js 文案，零后端改动）；
  - G-2 P-4 sparkline（app.py additive `total_by_root` + snapshots.js 趋势卡 sparkline）；
  - G-3 P-2 浏览历史下拉（workspace.js 时钟按钮 → 下拉面板 + style.css）；
  - G-4 P-3 面包屑 >6 层折叠（workspace.js renderBreadcrumb + style.css）；
  - G-5 P-9 代码卫生（cli.py 非法转义 raw string，零功能改动）；
  - G-6 P13-FIX（snapshots.py 锁竞态，50 轮 0/50）。
- **验收修复 2 件套（独立变更集 8）**：relate 路径栈对齐死循环（stage-b 既有缺陷，
  非下钻跳转卡死主线程）+ 面包屑展开态粘滞（G-4 同路径重渲染不复位）。
- **门禁**：unittest 317 / ResourceWarning 317 / node 43 / py_compile 0 /
  smoke 21/21 / u50–u67 全量 + u68a–u68d 全绿 / P13 专项 0/50（详见
  《问题核查复现与修改定位清单.md》五-H）。
- **视觉验收**：gpt-5.6-luna 判读 u68a–u68d 共 12 张截图（2 项首判 FAIL 经
  DOM 客观采样复核改判 PASS），主代理复核一致。
- **已知限制更新**：P-2/P-3/P-4/P-5/P-9/P13 全部销项（见上「已知限制」段）；2-2 七轮
  未复现继续登记；真机 G8 busy 按「请求发起态」口径不变。
- **归档**：`docs/exec/阶段G-交付报告_20260906.md`（见交付报告）；资料包重新生成。

## 阶段 H（发布后维护批次：残留跟踪 / 环境基线对齐 / 探针卫生 / 文档一致性，2026-09-06）

- **H-1 环境基线对齐（用户裁定①文档基线以实测为准）**：手册阶段 A 设计/状态行基线注记
  落定 **Python 3.11.8 / Flask 3.0.2 / Node v24.19.0**（原规划标注 3.14.3/3.1.3/25 为
  核查时点冻结值，A–G 七轮「待环境对齐」挂账收口）；README 环境说明同步；**零代码改动**。
- **H-2 2-2 第八轮跟踪**：u50 桩态两轮 residualDetected=false + u66 桩态双浏览器 10/10
  （+ u50 view 段第 1 轮）——**未复现**，登记「未复现」不关闭（红线：确定性复现不绿≠
  关闭依据）。
- **H-3 文档一致性**：README 阶段 G 段归档占位符 `2026XXXX` → 实际文件名
  `_20260906.md`；手册壹章挂账表 P-6/P-7/P-8 状态复查（备注/环境注记类，无过期表述）；
  资料包重新生成（含阶段 H 交付报告）。
- **H-4 u62 探针卫生**：rapid 组（连点 2 次）终态目标推导修正为双 toggle 语义
  （终态 ∈ {target, startTheme}——450ms 间隔恰落 `--dur-theme-expand:450ms` 边界时
  两次翻转各生效终态=startTheme，F/G rapid-450b/c 偶发即此边界断言口径错误，非应用
  回归）+ 终态等待 600ms→800ms；仅改探针，零应用代码；验收连续 3 轮 46/46 无需复跑。
- **H-5 全量门禁维持性回归**：unittest 317 / ResourceWarning 317 / node 43 /
  py_compile 0 / smoke 21/21 / u50–u68d 全量 / P13 专项 50 轮 0/50（G-6 修复后维持）。
- **H-6 交付报告与文档收尾**：`docs/exec/阶段H-交付报告_20260906.md`（六段结构）；
  核查清单追加五-I；阶段计划追加阶段 H 段。
- **已知限制更新**：P-6/P-7/P-8 仍为备注/环境注记类挂账（只复查登记不实现）；2-2 八轮
  未复现继续登记；真机 G8 busy 按「请求发起态」口径不变。
...
| `docs/问题核查资料_20260908/p2/manifest.json` | **P2 已交付证据清单**（5 证据项 / 11 张图片；含 A 类前后对照 frames+summary、B 类 screencast 像素帧、C 类三视口截图、红线 B 数据目录基线） |
| `scripts/dev/p02_view_frame_probe.mjs` | **P2 帧级探针**（视图切换残留的关闭依据工具；`--label prefix/postfix` 同一支跑前后对照） |
| `scripts/dev/p02_b_screencast.mjs` / `p02_c_viewport_shots.mjs` | P2 的 B 类（CDP screencast + 参照帧像素分类）与 C 类（三视口 + 画布像素统计）取证工具 |
