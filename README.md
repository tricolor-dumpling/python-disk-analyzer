...
- **完成**：进度条绿光扫过 + 条尾对勾描边 + 16 粒粒子庆祝（仅主页可见时播）+ **自动保存**（阶段D·D-2：「扫描完成自动保存」默认开启——缺键视为 ON；设置弹窗同步；自动保存失败错误可见 + 手动「立即保存/暂不保存」提示条恢复）。
...
- **Web 版（阶段D·D-2）**：全量扫描完成后自动保存（「扫描完成自动保存」默认开启——
  未显式存储视为 ON，设置弹窗同步）；自动保存失败错误可见 + 手动「立即保存 / 暂不保存」
  提示条恢复；auto_save 关闭时保留手动提示路径（既有流程不破坏）；
  快照管理页可「创建快照」与「撤销最近保存」。
