/* P4 红线 B 复核：用户真实数据目录跑测前后零变化（清单 + 尺寸 + mtime 三口径）
   用法：node docs/问题核查资料_20260908/p4/_verify_datadir_redline.mjs
   基线：同目录 _datadir_baseline_prerun.json（P4 跑测前采集，ConvertTo-Json 写出；
         口径与 P3 同名脚本一致，便于两阶段对照）
   说明：P4 全程以隔离数据目录跑测（LOCALAPPDATA / DSA_SNAPSHOT_DIR 双隔离，
         夹具在 %TEMP%），本脚本证伪「跑测期间真实目录被写入/删除/改动」。 */
import fs from "node:fs";
import path from "node:path";

const BASE = "docs/问题核查资料_20260908/p4/_datadir_baseline_prerun.json";
...
/* PowerShell 5.1 `Set-Content -Encoding UTF8` 写出 BOM，JSON.parse 不接受 → 先剥 BOM */
