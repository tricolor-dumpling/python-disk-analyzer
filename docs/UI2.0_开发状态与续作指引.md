...
更新时间：2026-08-30（**U4.3 完成会话更新**；执行事实以《UI2.0_开发执行手册.md》头部执行记录为准，含 U4.3 行——本文档仅为换机交接摘要）

## 当前状态

- **分支**：`ui2.0`（= U4.3 合并后最新提交，已推送 GitHub origin；`main` 未动，仍是 P12 归档完成线）。
- **提交链**：`5ec0061`(P12 尾) → `df01038`(U1.0) → `62281ab`(U1.1 实现) → `25faea2`(U1.1 验收记录) → `3d0e736`(换机交接文档) → `5466bde`(U1.2) → `83ebadd`(U1.2 顺手修复) → `b06c3ab`(U1.3) → `1e5d234`(U2.0) → `ffee6d1`(U2.1) → `3daa9d4`(U2.2) → `cc5317c`(U2.3) → `0cd0ef7`(U2.4) → `fa919c7`(U2.5) → `25f4f6a`(U2.5-doc 追记) → `a961e58`(U3.1) → `92ec485`/`2bb5d30`(U3.2a/b) → `8eff00f`(U3.2-doc) → `2e845f2`(U3.3) → `62e155b`(U3.4) → `56ce6b3`(U3.5) → `da5a06e`(U4.1) → `b3f5217`(U4.2) → **`<U4.3>`**（见下）。
- **进度**：U1.0–U1.3、U2.0–U2.5、U3.1–U3.5、U4.1–**U4.3 全部完成**（实现 + 验收；**批次四收口**——README v2.0.0、本手册标记「已完成」、《定稿》回补入库、死代码删除清单入执行记录）；**下一步 = 发版判定（手册「八、总验收清单」逐项勾选）/ 听安排**。
- ⚠️ 搁置/挂账（详见手册执行记录 + 核对单 §3）：P13（test_budget 并发竞态，禁碰 snapshots.py——复现率波动同 U4.2 记录，判定按「复现允许复跑」纪律）；**F07 历史下拉形态、N12 面包屑>6 层折叠（G2/G5 保持挂账——U4.3 未裁决，功能等价已达成，转发版备注语义）**；L3-5 sparkline 仍为降级差值卡（后端补逐次总量字段后启用）；u32 ⑧「50 次循环 DOM 节点±5」预存漂移（ui2.0 基线复现，非回归）；1920 常规档右栏满态内滚=说明项（§3.4 面板内滚允许；紧凑档保证仅 <820px 高）。
- 服务：**本会话结束停服**（Flask 5000 与静态服务 8771 已释放，端口空闲；续作起手命令见下）。⚠️ 真机旅程曾触发真实全量扫描（均已停止收束；Everything 索引 busy 窗口 5-15 分钟为环境常态——续作先看健康徽章再动浏览类验收）。
- 工作区未跟踪项仍为 `.venv/`、`dsh-image-gen/`（**《定稿》已于 U4.3 回补入库**——未跟踪三项减为两项，仍禁入提交）。

## 本次会话完成内容：U4.3 文档与版本收口（批次四收尾项）

验收口径与结果（详细记录在《docs/UI2.0_开发执行手册.md》头部 U4.3 执行记录）：

| 项 | 结果 | 证据 |
|---|---|---|
| README 升版 v2.0.0 | ✅ | 标题版本注记 + 新增「Web 界面（UI 2.0 · SpaceLens Pro）」章 9 小节（单屏 App Shell/子页面路由/主题三态/命令面板/键盘矩阵/工作台视图区/扫描与停止/零滚动紧凑档/本地启动）；TUI 等既有节保留并按现状校正；**无文档漂移**=u43 探针 ① 逐条断言 |
| docs/开发方案.md | ✅ | 进度区新增「UI 2.0 改版（U1.0–U4.3 全部完成 ✅）」小节（链接本手册/定稿/核对单）+ §六 进度日志 2026-08-30 行 |
| 《定稿》归档回补 | ✅ | U1.1 注记④核销：`docs/UI终版方案_SpaceLensPro视觉动效与功能补全_定稿.md`（v1.2）只读校验后 git add 入库 |
| 死代码删除 | ✅ | a) app.js 空壳物理删除（零引用）＋15 处模块头注释改述＋来源清单改指 icons.js；b) smoke legacy 残留注释清理（注册表仅 v2）；c) style.css 删 `.page-body-empty`×2（全覆盖扫描双通道 + token 核对，其余候选均为动态态 DOM 类）——清单入执行记录 |
| 门禁复跑 | ✅ | smoke **21/21**；unittest **266 OK**（本轮复跑即绿；P13 挂账不变，零 py 改动）；node **25/25**；hex **0**；u42 **39/39**（删除后总回归：①a 18.30/18.20ms、①b 0.0MB、①c 60.0fps、①d 360/P95 174.9ms、①e 21.8ms、P1-P7 全过、走查截图、G1/G3、console 0）；u41 **31/31**（键盘矩阵抽跑：stub 23+reduced 7+双档 2；真机段 --with-data 未跑——G8 惯例）；u43_doc_probe（新建）**48/48** |
| 本手册状态 | ✅ | 头部状态改「已完成」（U1.0–U4.3 全部完成）；§3.1 文件表 js/app.js 行与 smoke 行改述现状 |
| 挂账去向 | ✅ | G2/G5 不裁决保持挂账（转发版备注语义）；L3-5/P13/G8/G9 不变 |

**本次实施注记（下一会话须知）**：
1. **版本收口**：用户可见版本串 = 状态栏 `v2.0.0`（index.html；与 README 标题版本注记一致）；U4.3 起旧 `v0.9.0-p12c` 不再出现；下次升版改 index.html statusbar-left + README 头部两处。
2. **死代码纪律**：`web/static/js/app.js` 已物理删除——任何模块头注释不再出现「app.js」字面（改述为「U2.0 模块化迁入」）；u43 探针 ② 的 grep 断言即此口径（web + smoke + README 零命中）。
3. **style.css 死规则判定方法已记录**（u43 执行记录）：浏览器覆盖扫描（520 规则 × 桩/真实页双通道）+ 模板 token 存在性核对；后续若改模板结构，新增死规则用同法核查；hex 门禁仍为 0 只减不增。
4. **u43_doc_probe**（新建 `scripts/dev/u43_doc_probe.mjs`，48/48）：README×实现逐条/死代码 grep/hex/版本一致性/console 0；桩页键盘矩阵行为最小集复用 smoke 脚手架（设置弹窗无 data-close 按钮——用 Esc 栈关）；真实页 `#/compare` 会触发真实对比（G8 SDK 直扫口径：仅断言页头 DOM，勿等待完成态）。
5. **门禁命令**（不变）：smoke `node scripts/dev/u22_smoke_probe.mjs v2`（静态 8771）；unittest `.venv\Scripts\python.exe -W error::ResourceWarning -m unittest discover -s tests -t .`；node `node --test-isolation=none --test "scripts/dev/*.test.mjs"`；hex 计数见手册附录B。
6. 服务已停：Flask 5000 / 静态 8771 空闲；续作先查端口残留再启动（命令见「当前状态」节）。改 index.html 需重启 Flask（模板缓存），改 js/css 无需。

验收口径与结果（详细记录在《docs/UI2.0_开发执行手册.md》头部 U4.2 执行记录）：

| 项 | 结果 | 证据 |
|---|---|---|
| 附录B 基准复测（数值表） | ✅ | ①a 1000 块入场 P95 **18.0ms**/横扫 P95 **17.8ms**（≥50fps）；①c 5000 行滚动 **60.0fps**；①b 50 次下钻 heap Δ**0.0MB**+canvas=2/tooltip=1；①d 路由 token 360ms、换装完成 P95 **178.0ms**（全稳定 P95 426.9ms 记录值——含 240ms 入场收束）；①e 主题 VT clickToAttr **15.3ms** ≤450ms（VT 完成 478ms） |
| 性能红线 7 条 | ✅ 全过 | P1–P5 见基准值；P6 双档零滚动 14 项 + 1366 紧凑档右栏无内滚无溢出（修复后 stub/真机 656/656）；P7 getAnimations 白名单（修复 P12 遗留 6 处颜色类过渡——见手册） |
| 双主题全界面走查 | ✅ | 40 张截图目检合格（亮/暗 × 三页 × 两档 + 浮层族 + toast + 空态 + 灰度）——%TEMP%\u42_acc_shots |
| 灰度色盲检查 | ✅ | 对比页发散条/摘要卡灰度化——▲/▼/± 符号冗余可辨（亮/暗两档截图目检 + 符号存在断言） |
| 问题修复（≤2 轮） | ✅ | 第 1 轮：紧凑档右栏 CSS 收口（折叠次线条+卡内收紧）+ P7 过渡白名单；无第 2 轮 |
| G 裁决 | ✅ | **G1 裁决即修**（F06 启动恢复：browsePath 写 pds_last_browse_v1、start/evaluateEnvGate 恢复路径零额外请求、非法回落首根——真机 D:\虚拟机 实测恢复）；**G3 裁决即修**（F22 状态栏已选 N 项：list.js refreshSelectionUI 同源接线）；G2/G5 保持挂账（功能等价；形态项留 U4.3/发版备注） |
| 门禁 | ✅ | smoke v2 **21/21**（A0–A20）；unittest **265/266 OK**（P13 预存竞态复现 93 次——本会话复现率≈100%（全套 discover），单测/半套 35-50%；进程内预 import scan/sdk 后≈100%；**用户裁决：如实挂账提交**（2026-08-30）——根因=`.snapshot.lock` unlink 与 `_is_stale` read_text 的 Windows 共享冲突（无 FILE_SHARE_DELETE）→ 活 PID 锁残留 → 1s 重试窗耗尽；禁碰 snapshots.py，去向 backlog/P13）；node **25/25**；hex **0**（style.css；tokens.css 61 豁免） |
| u42 验收探针（新建） | **39/39** ✅ | `scripts/dev/u42_acc_probe.mjs`：基准表/红线 15 项/走查截图/灰度/G1/G3/console 0；真机抽查 5/5（基准页外独立执行——见注记） |
| 回归 | ✅ | u41 **31/31**（真机段沿用 U4.1 记录+本会话真机抽查 5/5）、u31 **38/38**、u25 **46/46**、u24 **34/34**（1 断言适配 padding 6/12）、u33 **31/31**、u34 **44/44**、u32 **45/46**（⑧ 预存漂移复现）、u35 **50/50**（真机段同 U3.5 记录） |

**本次实施注记（下一会话须知）**：
1. **u42 探针 ①b 已知偶发协议级卡死**：页内 50 次下钻循环 evaluate 偶发永不返回（浏览器/页面 CPU 归零；与基准页共存/独立页均可能触发，独立浏览器实例 + 95s 看门狗 + 重试一次 = 稳定方案）。**改 ①b 时勿回退为裸 evaluate**。
2. **u24 紧凑档 padding 断言**已按 U4.2 收口值适配（10/12 → **6/12**）——若后续再调紧凑档 padding，需同步该断言（u24 行 369）。
3. **P13 本轮复现率异常升高**（进程内预 import scan/sdk 后 ≈100%；轻载 35-50%——较 U1.x-U4.1 会话的「复现一次」明显波动）：根因=`.snapshot.lock` unlink 与 `_is_stale` read_text 的 Windows 共享冲突（无 FILE_SHARE_DELETE）→ 活 PID 锁残留 → 重试窗耗尽；**禁碰 snapshots.py**；unittest 判定仍按「P13 复现允许复跑」纪律执行；若 U4.3 时仍困扰，可考虑经 手册 §十 挂账格式升级为「确定性挂账」并注明本轮复现统计。
4. **G1 语义（F06）**：pds_last_browse_v1 = `{root,path}`（§3.2 键表）；browsePath **成功**才写（失败分支不写——真机「路径不存在」验证过）；启动时由 main.start() 读 + setCurrentRoot(root)/输入框=path，`evaluateEnvGate` ready 分支以恢复路径为 browse 目标（与旧首根浏览**同一次** browse——零额外请求，u20 Network 纪律保持）；非法/缺失回落首根。
5. **G3 语义（F22）**：`#statusbar-selected` 在 index.html 壳层（smoke 脚手架无→`renderStatusbarSelection` 空守卫）；刷新源=list.js `refreshSelectionUI`（APP_STATE.selection.keys.length）；多选清零→隐藏。
6. **紧凑档折叠清单**（max-height:820px；**u24 textContent 断言语义不受影响**——display:none 保留文本）：`#overview-meta`（最近扫描副行）/`.overview-empty span`（空态指引二行——主标题+动作保留）/`.snapshot-mini-card .status-line`/`.compare-mini-empty span`；另 `.donut-legend-row` 单行、卡 padding 6/12、btn-sm 3/8、chip 2/8、gap 4。
7. **1920×1080 常规档右栏**在「扫描完成+保存提示条」满态下内滚≈120-430px（视状态）：§3.4 面板内滚允许（仅 1366 紧凑档保证无内滚）；走查截图记录为准，勿在 U4.3 误判为回归。
8. 服务已停：Flask 5000 / 静态 8771 空闲；续作先查端口残留再启动（命令见「当前状态」节）。**改 index.html / 模板缓存**：debug=False 需重启 Flask；改 js/css 无需（ETag 304）。

## 本次会话完成内容：U4.2 性能与视觉总验收（批次四收口前验收项）

## 本次会话完成内容：U4.1 无障碍与键盘矩阵（批次四首个）

验收口径与结果（详细记录在《docs/UI2.0_开发执行手册.md》头部 U4.1 执行记录）：

| 项 | 结果 | 证据 |
|---|---|---|
| 键盘矩阵全量（定稿 7.4） | ✅ | `keys.js`（共享守卫：isEditableTarget/isTypingEvent——输入框/可编辑/isComposing 一律忽略，与 Backspace/R 同口径）；`keyboard.js`（/ 聚焦筛选框、g c/g s 连按跳页 800ms 窗口、treemap 聚焦后 ↑↓←→ 最近邻 + Enter）；Ctrl⌘K/Esc/R/Backspace 既有不移 |
| treemap 焦点块 | ✅ | `nearestFocusIndex` 纯函数（候选中心严格前进+欧氏最近；边界/单块/无 tiles=-1 不动）；静态描边 2px primary（不动画=reduced 不降级）；`APP_STATE.treemap.focusIdx` 单一来源；Enter=activateFocus（清待发单击→onClick 既有链；文件/合并块 0 请求、双击窗互斥） |
| 焦点环/aria/焦点管理 | ✅ | `:focus-visible` 2px primary offset 2 全局核查（补 .treemap-wrap/.strip-block[inset 环，overflow 裁剪防护]）；treemap 容器 aria-label 摘要（当前目录 X，共 N 项，最大子项…）；路由焦点管理/toast aria-live 核对既有；补缺不破既有 |
| 纯键盘全旅程+录屏 | ✅ | u41 ⑨：浏览（Tab/方向键/Enter/Backspace 发起态/筛选）→扫描（发起态+收束）→对比（g c）→设置（Ctrl K/Esc）；**录屏 webm 留档** %TEMP%\u41_acc_shots\journey\page@378cd967864ab359f9b5875b25dbed5d.webm；焦点环亮/暗/红截图 3 张 |
| reduced-motion 降级总表 | ✅ | L0×5/L1×5/L2×11/L3×9 + 总表逐行复核（循环 .01ms/入场直显/转场直切/L2-4 不播/toast·shake 功能性保留 ≤120ms）；键盘导航功能性不降级；焦点环静态 |
| 门禁 | ✅ | smoke v2 **21/21**（A20 新接入）；unittest **266 OK**（P13 首轮复现一次复跑即绿——零 py 改动）；node **25/25**（+5 nearestFocusIndex）；hex **0**；u41 探针 **46/46** |
| u41 验收探针（新建） | **46/46** ✅ | `scripts/dev/u41_acc_probe.mjs`：桩态 23（数学/移动/边界/像素描边/容器焦点环 computed/Enter 恰 1 次/Backspace goUp 恰 1 次// 聚焦/g 序列守卫/console 0）＋reduced 7（键盘保留/描边静态/CSSOM 规则常驻/Enter 保留）＋双档零滚动 2（焦点态）＋真机 16（全旅程+录屏+亮暗截图） |
| 回归 | ✅ | u31 **38/38**（⑧ `/` 语义适配→聚焦筛选框）；u34 **44/44**；u35 **50/50**；u24 **34/34**；u33 **31/31**；u32 **45/46**（⑧ 基线预存——见上） |
| 视觉多模态 | ✅ | 焦点环亮/暗截图目检（容器 2px 环 + 焦点块描边，双主题可见）；双档零滚动焦点态实测 |

**本次实施注记（下一会话须知）**：
1. **键盘矩阵挂点**：`keyboard.js` 在 main.js start() 一次绑定；treemap 方向键/Enter 仅在 `document.activeElement` 位于 `#treemap-wrap` 内时消费（列表行 Enter 归 list.js、路径输入 Enter 归原有——互不干扰）；`/` 与 g 序列共用 keys.js 守卫 + 弹窗栈守卫（hasOpenModal）。
2. **`/` 语义（偏差注记①）**：定稿=聚焦筛选框；源码 `.browse-filters` 恒可见（无视图联动）——/ 直接聚焦无需切视图；其他路由空守卫忽略。旧「/ 聚焦顶栏搜索框（btn-palette）」语义废止（u31 ⑧ 已适配；Ctrl⌘K 仍是顶栏搜索的唯一快捷键）。
3. **焦点块状态**：`APP_STATE.treemap.focusIdx`（-1=无焦点）由 treemap.js 维护（单一来源）；容器 focusin 时若未聚焦→0（最大块）；setTiles 数据换代 key 跟随/失效归位；moveFocus 边界=无候选不动；焦点块=终帧静态描边（与 hover 描边同几何，焦点不依赖动画）。
4. **reduced-motion Chromium 焦点环注记（偏差注记③）**：reduced 模式下 div[tabindex] 的系统焦点环会替代表格化 author outline（2.67px 环仍可见）；正常模式 2px primary 实测（②b）；探针在 reduced 断作者规则 CSSOM 常驻 + 匹配态（勿退化为断 computed 值）。
5. **真机全旅程口径（偏差注记②）**：本机 Everything SDK 直扫分钟级+查询串行化（G8 升级）——扫描收束后 SDK 长忙（browse 排队/409 数百秒），真机「扫描→浏览」序浏览步不可达；旅程采用 **浏览→扫描→对比→设置** 序（键盘全程不减），真机下钻/返回降级为「请求发起态+录屏」（用户裁决：完成态语义由桩态 ③⑥b/A20/node 确定性验证）。续作真机类验收：先看健康徽章「已就绪」（Everything busy 窗口 5-15 分钟）再开跑，否则白等。
6. **探针等待纪律**：Playwright `waitForFunction(fn, arg, {timeout})`——传错签名（options 当 arg）会静默用默认 30s；本仓库探针均已用 `, null, { timeout }` 三参式；请求采集监听必须**先挂后按**（fetch 随 keydown 同步发起，后挂会漏）。
7. **u32 ⑧ 预存**：50 次停止/开始循环 DOM ±5 断言在 ui2.0 基线（56ce6b3 worktree）复现 before=466/after=459——漂移源于 U3.x 模板收敛，非本轮。
8. 服务已停：Flask 5000 / 静态 8771 空闲；续作先查端口残留再启动（命令见「当前状态」节）。改 index.html 需重启 Flask（模板缓存），改 js/css 无需。

（以下为 U3.5 及更早会话记录，随本文件历史保留：）
...
验收口径与结果（详细记录在《docs/UI2.0_开发执行手册.md》头部 U3.4 执行记录）：

| 项 | 结果 | 证据 |
|---|---|---|
| 页面装配（§3.3/§3.5） | ✅ | 页头 64px 行式（标题「历史对比」+ 基线 datalist + 目标只读 + 开始对比）→ 摘要 3 卡 96px（L1-4 count-up）→ 红绿发散图 240px（L3-6）→ 表格 flex:1 面板内滚（F19 定位+复制路径）；双档零滚动实测 |
| **数据源口径核对结论** | ✅ | app.py api_compare 执行核对：/api/compare「当前」侧 = **fullscan.result(root) 缓存 or SDK 直扫**（非快照文件）→ **「目标=同盘符最新快照」为展示口径（只读标识性文案）**，对比目标实为当前磁盘状态；零后端改动（偏差注记①；页面 label title 已注明） |
| L3-6 发散条 | ✅ | 中轴基线；增长条向左红 --up / 缩减条向右绿 --down；条宽=|Δ|/max|Δ|×半轨（≤50%）；中轴生长 500ms（`--dur-diverge` 新增，仅 transform scaleX）；徽标 pop-in scale .8→1（--dur-3 + --ease-spring）；▲/▼± 冗余；reduced 直显 |
| 三入口预填 | ✅ | 趋势卡（trio + **结果共享**：趋势卡缓存整份 report → 落地即渲染不重发）/ 迷你条目（trio）/ 直达（默认=当前盘最新快照）；均自动执行（骨架屏 → 摘要 → 图 → 表，定稿 6.4） |
| 结果缓存/回灌 | ✅ | APP_STATE.compare.result（root/baseline/report）；路由往返回灌不重发；markNavDot 仅真实完成挂点（回灌不挂防残留） |
| 空态（定稿 6.5） | ✅ | 「选择一份基线快照，开始对比两个时间点的空间变化。」；无快照不自动对比 + 状态行引导 |
| 旧卡迁移 | ✅ | 工作台「历史对比」卡移除（F18；ids 随迁 #/compare），主页仅留「最近对比」迷你卡；palette「开始对比」→ #/compare 预填默认；compareSnapshots 保持 DOM 无关（u31 跨页触发路径）；W2.4/W2.13 语义保留 + 锁释放自动重试（上限 3）；wipeData → resetCompareData 全量复位；scan.js/settings.js 空守卫迁移 |
| 顺手修复 | ✅ | workspace.js browsePath 失败分支 `$("browse-root")` 空守卫（真实页启动期对比自动执行与 init 链浏览竞争 SDK 锁 → 子页面 TypeError，真机复现后修复——偏差注记②） |
| 门禁 | ✅ | smoke v2 **19/19**（A18 新接入）；unittest **266 OK**（P13 复现一次复跑即绿——零 py 改动）；node **20/20**；hex **0**；tokens.css +1（--dur-diverge:500ms） |
| u34 验收探针（新建） | **48/48** ✅ | 桩态 40（装配/三入口/只读/摘要 count-up/发散图 L3-6 全参数/表格 F19 open-path 实调/空态/骨架/50 次对比无泄漏/双档零滚动/截图）+ reduced 4 + 真机 4（真实会话装配+挂载自动对比发起态/console 0——⚠️ 本环境 SDK 直扫分钟级（预存环境性，见注记 8）） |
| 回归 | ✅ | u33 **31/31**、u31 **38/38**（⑤② 触发改经 compareSnapshots 跨页路径）、u24 **34/34 + 扫描态 3/3**（最近对比摘要经 #/compare 真实路径）、u25 **46/46** |
| 视觉多模态 | ✅ | 亮 1366/暗 1366/亮 1920/空态/窄屏 800×700 截图目检合格（发散图几何/摘要卡/表格/空态；修复条宽溢轴初版缺陷） |

**本次实施注记（下一会话须知）**：
1. **「目标=同盘符最新快照」是展示口径**（偏差注记①）：/api/compare 当前侧=fullscan.result 缓存 or SDK 直扫；目标快照路径只用于只读标签，**从不参与请求**；真机对比完成需后台全量结果缓存（本环境 SDK 直扫 C:\ 实测 >300s 超时——真机阶段在真扫完成后验证）。
2. **结果缓存共享链**：趋势卡（fetchTrendCompare 缓存 report）→ prefillAndGoCompare 写 APP_STATE.compare.result → #/compare 挂载 resultCacheMatch 命中即渲染（不重发）——A17/u33 ③b「往返不重发」计数口径靠此保持；**改动时勿拆该共享**。
3. **compareSnapshots DOM 无关**：页面未挂载（如 u31 跨页触发）只记账 lastSummary/result + markNavDot；页面挂载才渲染 UI——跨页调用入口保留。
4. **409 自动重试**：pds:scan 完成分支 + scanRetries 上限 3（防持续 409 轮询风暴）；状态行如实文案「完成后将自动重试（或稍后手动点击）」；成功/手动发起复位计数。
5. **对比页联动清空**：settings wipeData → resetCompareData（结果/迷你摘要复位 + 对比页在位回空态）——勿回退为直取 #compare-result DOM。
6. **启动期竞态修复**：browsePath 失败分支已空守卫（子页面无 #browse-root）；U3.5 若新增启动期 API 自动执行，注意 init 链浏览/对比的 SDK 锁竞争（409 属预存行为）。
7. 服务已停（用户要求延续）：Flask 5000 / 静态 8771 均空闲；续作先查端口残留再启动。改 index.html/静态资源无需重启（Flask 按 ETag 重发），改 py 才需重启 Flask。
8. **本环境 Everything SDK 直扫分钟级**（偏差注记③）：/api/compare 无后台结果缓存时直扫任意目录（连 19 文件单目录）>300s 超时——**真机「对比完成」在本机验收窗口内不可达**（预存环境性，正常桌面不受影响）；真机阶段如实断言「发起态」（状态行 正在对比/锁定重试 + console 0 + 零滚动），完整对比渲染链以桩态确定性验证。U3.5 若做真机对比类验收，沿用该口径或先确保 fullscan.result 缓存就绪。

## 本次会话完成内容：U3.3 快照管理页（#/snapshots）

验收口径与结果（详细记录在《docs/UI2.0_开发执行手册.md》头部 U3.3 执行记录）：

| 项 | 结果 | 证据 |
|---|---|---|
| 页面装配（§3.3/§3.5） | ✅ | 页头 64px 行式（标题+创建快照 F15+撤销最近保存 F16）→ 趋势区 128px 趋势卡×2（N07）→ 会话分组列表 flex:1 面板内滚（F17） |
| **字段核对结论** | ✅ | `/api/snapshots` 会话数据**无逐次总量**（app.py api_snapshots + session.py 双核对）→ **L3-5 sparkline 降级为「两快照对比差值卡」**（无折线，保留 ▲/▼ 与百分比；零后端改动；smoke/探针断言 0 svg） |
| N07 趋势卡 | ✅ | 较昨日=同盘符 0<Δt≤24h 最近一份；较上周=(24h,7d] 最近一份（排除 ≤24h——两卡基线去重，偏差注记①）；目标=该盘最新快照；无合适基线→「暂无对比基线」；Δ=复用 /api/compare（各卡恰 1 次；trendCache+inflight 防重发；路由往返回灌不重发）；点击卡→state.compare 预填+跳 #/compare |
| F15/F16 页头 | ✅ | 创建快照=复用保存流程（scan.js saveSnapshot 注入）；可用性=isSaveAvailable() 镜像扫描卡保存派生（N06 置灰+提示）；撤销=undoLastSave 确认弹窗流；`#btn-undo-save` 同位迁页头 |
| F17 列表 | ✅ | 会话分组/auto·manual 标签/逐盘「对比此快照」=预填+跳转（§6.4 跨页形态）/跳过原因 tooltip（红线 #7 SKIP_REASON_TEXT）；三处空态（定稿 6.5） |
| 工作台迷你卡收敛 | ✅ | 折叠区/撤销/刷新移除；空态补「打开快照管理」单按钮（N06 要求）；scan.js/compare.js 空守卫补漏（原绑定在工作台挂载 TypeError）；wipeData 改 applySnapshotsView 全量复位 |
| 门禁 | ✅ | smoke v2 **18/18**（A17 新接入；A8 只增不删）；unittest **266 OK**（P13 首轮复现 1 fail、复跑即绿——零 py 改动）；node **20/20**；hex **0**；tokens 零新增 |
| u33 验收探针（新建） | **35/35** ✅ | 桩态 31 + 真实页 4（真实 2 会话装配/双档零滚动/console 0；browse/compare 并发 409 资源日志按 U3.2 注记 4 过滤） |
| 回归 | ✅ | u21 ✓（errors 0；快照断言适配迷你条目）、u24 **34/34**（折叠区/空态断言按迁移适配）、u25 **46/46**、u31 **38/38** |
| 视觉多模态 | ✅ | 亮/暗 1366 + 1920 + 800×700 窄屏 + 真实页截图目检合格（差值卡/列表/跳过原因/空态；布局修正核实 deltaX=labelX=33 左对齐） |

**本次实施注记（下一会话须知）**：
1. **趋势卡口径（偏差注记①）**：较上周窗口 = (24h, 7d]（排除 ≤24h）——若照字面「≤7d 最近一份」会与较昨日同基线、两卡信息重复；U3.4 消耗预填时以 `APP_STATE.compare.{baseline,root,target}` 为准（root 为 §3.2 形状之外附加键）。
2. **setSnapshotsActions 注入模式**（main.js）：快照页页头动作/可用性经注入（防 scan↔snapshots 环——scan.js→snapshots.js 单向依赖保持）；新页面如需复用 scan.js 能力，照此模式，勿反向 import。
3. **回灌纪律**：snapshots 页 mount = applySnapshotsView（缓存回灌不重发 compare）；trendCache（模块级 Map，键=槽:根:基线）跨路由持久；趋势卡点击/逐盘对比只预填+跳转（#/compare 仍占位至 U3.4——U3.4 落地后自动闭环）。
4. **mini 卡无 #snapshot-list/#btn-undo-save/#btn-refresh-snapshots/#btn-snapshot-expand**——全新探针勿引用（u21/u24 已适配）；完整列表在 #/snapshots（#snapshot-list 同 id）。
5. **真实页趋势卡**：慢 SDK 直扫/扫描在途时 compare 可能 409 或长挂——卡片按 err/pending 呈现（非前端缺陷）；真实页 console 断言过滤资源状态日志（U3.2 注记 4口径）。
6. 服务已停（用户要求延续）：Flask 5000 / 静态 8771 均空闲；续作先查端口残留再启动。
...
6. 服务已停（用户要求）：Flask 5000 / 静态 8771 均空闲；续作先查端口残留再启动（命令见「当前状态」节）。改 index.html/静态资源无需重启（Flask 按 ETag 重发），改 py 才需重启 Flask。
...
`U1.x 地基 + U2.0-U2.5 核心可视化`（已完成）→ `U3.1 顶栏/命令面板`（已完成）→ `U3.2 扫描卡＋停止接口`（已完成）→ `U3.3 快照页`（已完成）→ `U3.4 对比工作台页`（已完成）→ **`U3.5 弹窗族与引导收尾`（下一步，批次三闸门项）**；批次三闸门 → `U4.1 无障碍 → U4.2 性能与视觉验收（含 F22 状态栏「已选 N 项」补接）→ U4.3 文档与版本收口 → 发版判定（v2.0.0）`。单人执行建议：下一步 U3.5（设置弹窗主题三态 + 危险区 L2-10 倒计时 + toast L2-6 升级 + F/N 全表核对）。
