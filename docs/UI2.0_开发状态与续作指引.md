...
更新时间：2026-08-30（**U4.1 完成会话更新**；执行事实以《UI2.0_开发执行手册.md》头部执行记录为准，含 U4.1 行——本文档仅为换机交接摘要）

## 当前状态

- **分支**：`ui2.0`（= U4.1 合并后最新提交，已推送 GitHub origin；`main` 未动，仍是 P12 归档完成线）。
...
- **进度**：U1.0–U1.3、U2.0–U2.5、U3.1–U3.5、**U4.1 全部完成**（实现 + 验收；**G4 键盘矩阵核销**——核对单 §3 G4 行已更新）；**下一步 = U4.2（性能与视觉总验收）**。
- ⚠️ 搁置/挂账（详见手册执行记录 + 核对单 §3）：P13（test_budget 并发竞态，禁碰 snapshots.py）；F22 状态栏「已选 N 项」未接、F06 启动恢复 pds_last_browse_v1、F07 历史下拉形态、N12 面包屑>6 层折叠（挂账 G1/G2/G3/G5，U4.x 收口）；L3-5 sparkline 仍为降级差值卡（后端补逐次总量字段后启用）；u32 ⑧「50 次循环 DOM 节点±5」before=466/after=459 —— **ui2.0 基线逐字节复现 = 预存漂移**（非本轮回归，核对单/执行记录注记）。
