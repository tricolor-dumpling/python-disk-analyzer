...
> ✅ **状态：已完成**（2026-08-30 · U1.0–U4.3 全部完成并通过门禁；发版判定依据见「八、总验收清单」）。
>
> ✅ 执行记录：（每个工作项完成后在此追加一行，格式仿 P12：`U1.1：完成内容摘要｜测试结果｜遗留挂账`）
>
> - **U4.3：文档与版本收口（批次四收尾项；实现 + 验收完成）。** **README 升版 v2.0.0**（标题版本注记 + 新增「Web 界面（UI 2.0 · SpaceLens Pro）」章 9 小节：单屏 App Shell 布局（顶栏 60px 八组/路由视区/状态栏 32px/右栏 300px 四卡）、子页面路由表（`#/` 工作台 / `#/compare` 对比工作台 / `#/snapshots` 快照管理 + 定稿 6.5 空态文案表）、主题三态（顶栏亮↔暗显式切换 + 设置三态单选 + `pds_theme_v1` + 防闪烁/实时跟随）、命令面板（Ctrl/⌘K；六分组：页面/盘符/最近访问/浏览历史/快照/命令）、键盘矩阵表（Ctrl/⌘K、Esc、R、Backspace、Enter、`/`、`g c/g s`、↑↓←→——定稿 7.4 实现与 README 逐条一致）、工作台视图区（treemap 默认/10 色哈希取色/合并阈值 −/+ 默认 24/迷你条带/排行表格/多选 Shift 范围选/CSV 导出/虚拟滚动/全屏）、扫描控制与停止（D10 全语义：开始→进度条+计时+盘符 chips→红描边「停止」→完成绿光+粒子 16 粒+保存提示→中止 toast「已停止，已完成部分可浏览」；OPTIONS 特性探测；`stop_reason user|shutdown` 隔离；重开自动恢复 K7）、布局保证（两档零滚动 + 1366×768 紧凑档折叠清单 + P7 动画红线与已知例外）、本地启动（`python app.py[--no-browser]`/实例复用）；TUI/快照/非交互/部署/FAQ 各节保留并按现状校正（unittest 基线 266 + P13 挂账说明、前端门禁 smoke 21 项/node 25、项目结构补 app.py/fullscan/session/datadir/web）——**无文档漂移验收**：u43_doc_probe ①README 陈述 × 实现逐条 15 项 + ⑤桩页行为最小集（顶栏八组/路由三页/主题三态/`/`/g c/g s/Ctrl K/主题切换+持久化）+ ⑥真实页壳/品牌/版本 v2.0.0/零滚动/console 0。**docs/开发方案.md** 进度区新增「UI 2.0 改版（SpaceLens Pro；U1.0–U4.3 全部完成 ✅）」小节（链接本手册/《定稿》/核对单）+ §六 进度日志追加 2026-08-30 行（§十 约定）。**《定稿》归档回补**（U1.1 偏差注记④核销）：本地未跟踪 `docs/UI终版方案_SpaceLensPro视觉动效与功能补全_定稿.md` 只读校验（v1.2 标题/修订记录/D1-D15 决策表/§7.4 键盘矩阵/§6.5 空态表齐全，无生成残留）后 git add 入库；核对单/执行记录引用路径已核对一致（本手册头部「依据」与指引文档均用正式路径）。**死代码删除清单（逐项）**：a) `web/static/js/app.js` 空壳（U2.0 起过渡注释壳——index.html 仅引 main.js；grep 全厂确认零引用后物理删除）+ 模块头 15 处「从 app.js 迁入/迁出」注释改述为「U2.0 模块化迁入」（保留迁移信息、清除指向已删文件的引用）+ `web/static/assets/来源清单.md` 图标位置改指 `js/app/icons.js`；b) legacy smoke suite 残留（U2.5 已删 ASSERTIONS 注册表/`?suite=legacy`—本项把 2 处陈旧注释改述为现状，注册表仅 v2 未动，u22 21/21 复跑通过）；c) style.css 死规则（**U1.3 注记③删除清单核查 + 全覆盖扫描人工判定**：删 `.page-body-empty`/`.page-body-empty .empty-state` 2 条——U2.1 占位页时代类，U3.3/U3.4 全量填充后无生成方（浏览器覆盖扫描 520 规则 × 桩/真实页双通道 + 模板 token 存在性逐条核对：其余候选全部为动态态 DOM 类（hover/扫描态/空态/聚焦态/数据态），均有生成方，保留）；「pages」分区注释「compare/snapshots 占位」改述现状；hex 0 只减不增）。**门禁复跑（删除后）**：smoke v2 **21/21**；unittest **266 OK**（本轮复跑即绿——P13 预存竞态挂账不变，复现率波动同 U4.2 记录，判定仍按「P13 复现允许复跑」纪律；零 py 改动）；node **25/25**；hex **0**（style.css；tokens.css 豁免）；u42_acc_probe **39/39**（删除后总回归：①a 18.30/18.20ms、①b Δ0.0MB、①c 60.0fps、①d token 360/P95 174.9ms、①e 21.8ms、P1–P7 全过、走查截图 40 张、G1/G3、console 0）；u41 **31/31**（键盘矩阵 A20 语义抽跑：stub 23 + reduced 7 + 双档零滚动 2——真机段 `--with-data` 未跑，G8 环境口径沿用）；新探针 **u43_doc_probe 48/48**（README×实现/死代码 grep/hex/版本一致性/console 0 全项）。**挂账去向**：G2（F07 历史下拉形态）/G5（N12 面包屑 >6 层折叠）本项不裁决→保持挂账（功能等价已达成；与 U4.2 处置一致，转「发版备注」语义）；L3-5 sparkline 降级差值卡（后端补逐次总量字段后启用）；P13（backlog/P13）；环境性 G8/G9（核对单）。**本手册状态：已完成**（U1.0–U4.3 全部完成；发版判定依据见「八、总验收清单」）。
>
> - **U4.2：性能与视觉总验收（批次四；实现/修复 + 验收完成；G1/G3 核销）。** **附录B 全部基准复测**（数值表见下；口径均沿用既有探针）：
>   ①a 1000 块 mock（console 注入 `state.treemap.tiles`→renderTreemapFromState，基准后恢复真实渲染）：入场 **P95 18.0ms**（n=80）/ hover 横扫 **P95 17.8ms**（n=101）——≥50fps（u22 ④ 同口径）；①c 5000 行 mock 滚动 **60.0fps**（u25 ⑥ 口径，60.5fps 于 u25 自身复测）；①b 50 次下钻/返回：**heap Δ0.0MB**（performance.memory）+ canvas=2/tooltip=1/单视图（u23 Phase B 口径，40s）；①d 路由 50 次：**token 口径 360ms**（--dur-1 120 pageOut + --dur-page-in 240 pageIn 读值），hashchange→换装完成 **P95 178.0ms**（avg 157.9），全稳定（含 pageIn 收束）P95 426.9ms（记录值——u21 ① 同口径：换装完成为准）；①e 主题切换 VT：vtCalled=1、**clickToAttr 15.3ms** ≤450ms、VT 完成 478ms（动画参数 450ms；u11 先例 470-481ms）。**性能红线 7 条逐条全过**：P1 1000 矩形 ≥50fps ✓；P2 主题 ≤450ms ✓（VT 路径）；P3 路由 ≤360ms ✓（120+240 token 口径+换装完成 P95）；P4 50 次下钻无泄漏 ✓（Δ0.0MB）；P5 5000 行 ≥50fps ✓（60.0）；**P6 零滚动恒成立 ✓（双档 × 主页/对比/快照 × toast/弹窗/焦点态 14 项全过 + 1366×768 紧凑档右栏无内滚无溢出——修复后 stub 数据态 656/656、真机 656/656）**；**P7 动画仅 transform/opacity ✓**（getAnimations 终核发现并修复 P12 遗留颜色类过渡：`.btn/.icon-btn/.chip/.badge`（background/border-color/color/box-shadow）、`.trend-card`、`.topbar-search`、`.theme-opt span`、`.strip-block`（filter）共 6 处/10 规则改为瞬态切换或移除——仅保留 transform/opacity 过渡；已知例外记录：width（L1-3 占比条/进度条——§3.5 定稿参数）、background-position（扫描斜纹）、stroke-dashoffset（顶栏扫描环进度弧，SVG 非布局属性））。**问题清单分级**：①[布局·已修] 1366 紧凑档右栏内滚（实测 stub 数据态 697/896→656/656：次级文案/状态行紧凑折叠——`#overview-meta`/`overview-empty span`/`snapshot-mini status-line`/`compare-mini-empty span`（主文案+动作保留，DOM 文本不变→u24 textContent 口径零影响）+ 卡内收紧 padding 6/12、row/status/chips 边距、btn-sm 3/8、chip 2/8、图例单行、gap 4）+ 1920 常规档满态（扫描完成+提示条）右栏内滚=说明项（§3.4 面板内滚允许；紧凑保证仅 <820px 高）；②[红线·已修] P7 见上；③[说明] 保存提示条态内滚（临时态）；④[说明] 紧凑档空态指引二行折叠（≥820px 完整显示）。**G1 裁决即修**（F06 启动恢复 `pds_last_browse_v1`：browsePath 成功写 {root,path}（失败不写）；start() 读取恢复 `setCurrentRoot`+输入框、evaluateEnvGate 同一次 browse 加载恢复路径（**零额外请求**——Network 时序纪律保持）；非法/缺失回落首根 D:\；真机实测浏览 D:\虚拟机→reload 恢复 ✓）；**G3 裁决即修**（F22 状态栏「已选 N 项」：statusbar.js renderStatusbarSelection + list.js refreshSelectionUI 同源接线（APP_STATE.selection），选中显示/清零隐藏；smoke 脚手架空守卫）。**G2（F07 历史下拉形态）/G5（N12 面包屑>6 层折叠）不裁决→保持挂账**（功能等价已达成（chips+面板数据源 / home 语义）；形态收口项留 **U4.3/发版备注**）。⏱️ 验收探针 `scripts/dev/u42_acc_probe.mjs` **新建 39/39**（阶段①基准数值表 6 项 + 阶段②红线 15 项 + 阶段③双主题全界面走查截图 40 张（亮/暗 × 三页 × 两档 + 浮层族（面板/popover/引导/设置/清空/confirm）+ toast 成/败 + 空态 4 + 灰度 2×2 符号冗余 ▲/▼/± 目检可辨）+ 阶段④console 0 + 阶段⑤G1/G3 核销 3 项；①b 独立浏览器实例+95s 看门狗+重试——**已知偶发协议级卡死**（页内 50 次循环 evaluate 永不返回且 CPU 归零；独立实例 3/3 稳定——u23 同构循环在本机曾复现，见 _tmp_dbg4 记录）；**真机抽查 5/5**（健康徽章就绪/路由 mount 真机 ≤360ms/双档零滚动/G1 真机恢复链路/console 0——G8 口径：真实 SDK 浏览 8s 级，条件等待）。**门禁**：smoke v2 **21/21**（A0–A20）；unittest **265/266 OK**（唯一失败=P13 预存竞态——本会话复现 **93 次**（全套 discover 连跑 0 绿；单测/半套 35-50%，进程内预 import scan/sdk 后≈100%：`.snapshot.lock` unlink 与 `_is_stale` read_text 的 Windows 共享冲突（无 FILE_SHARE_DELETE）→ 活 PID 锁残留 → 1s 重试窗耗尽；同 P1.0 挂账 backlog/P13，**用户裁决：如实挂账提交**（2026-08-30），**零 py 改动**（snapshots.py/compare.py/scan.py/app.py 均未触碰；其余 265 用例每次全绿））；node **25/25**；hex **0**（style.css 终核；tokens.css 61 豁免）。**回归全绿**：u41 **31/31**（桩+reduced+双档；真机段同 U4.1 记录口径——本会话另做真机抽查 5/5 佐证）、u31 **38/38**、u25 **46/46**（list.js 被触——50 次切换 DOM 无增长）、u24 **34/34**（1 断言按 U4.2 紧凑档 padding 6/12 适配——见 u24 注记）、u33 **31/31**、u34 **44/44**、u32 **45/46**（⑧ 50 次停止/开始 DOM ±5：before=468/after=461——**预存漂移同模式复现**（ui2.0 基线 466/459），值偏移 +2 为 U4.2 状态栏 span 增项，非本轮回归）、u35 **50/50**（真机段同 U3.5 记录）。⚠️偏差注记：①「路由 ≤360ms」以 u21 ① 口径（token 名义 120+240 + 换装完成实测）判定；全稳定实测 426.9ms（含 240ms 入场动画收束）数值入表——若以「稳定态」判读则超预算，记为已知口径差异；②紧凑档空态/状态行折叠（见问题清单③④——定稿 6.5 文案全量保留于 DOM 与 ≥820px 档）；③u24 紧凑档 padding 断言 10/12→6/12（U4.2 收口值）；④`#statusbar-selected` 仅在真壳（index.html）存在，smoke 脚手架无（空守卫口径）；⑤真机阶段沿用 G8 口径（SDK 直扫分钟级——实测定 8s 级可用，路由/健康/G1 真机链路实测通过）。
> - **U4.1：无障碍与键盘矩阵（批次四首个；实现 + 验收完成；G4 核销）。** **键盘矩阵全量落地（定稿 7.4）**——`js/app/keys.js`（新建·叶子·共享守卫：`isEditableTarget`/`isTypingEvent`——目标输入框/可编辑元素或 `isComposing` 一律忽略，与 U2.3 Backspace/U3.1 R 同口径，零依赖零环）＋`js/app/keyboard.js`（新建·键盘矩阵接线，main.js bindKeyboard 一次绑定）：treemap 聚焦后（`#treemap-wrap` tabindex=0；focusin 从最大块起）↑↓←→ **最近邻移动焦点块**（`nearestFocusIndex` 纯函数——按当前视口布局坐标、候选中心向严格前进、欧氏距离最近、无候选=-1 不动（边界/单块/无 tiles 守卫）；node 5 用例）；焦点块指示=**静态描边**（drawFocusRing 终帧 2px primary，不动画——reduced 不降级）；**Enter 下钻=单击语义**（activateFocus：清待发单击+走 opts.onClick 既有下钻链；文件/合并块 0 请求、不产生第三次请求）；`/` **聚焦筛选框**（#browse-filter；⚠️偏差注记①：手册「筛选行仅排行/表格视图可见」与源码不符——`.browse-filters` 恒可见（U2.0 模板无视图联动），/ 直接聚焦无需切视图；其他路由空守卫忽略；旧「聚焦 btn-palette」语义废止——u31 ⑧ 已适配）；**`g`+`c`/`g`+`s` 连按跳页**（vim 风格：首次 g 进 800ms 窗口，c→#/compare、s→#/snapshots；超时/其他键/修饰键重置；弹窗栈打开时忽略——手册「可选实现」已实现以核销 G4）；`Ctrl/⌘K`、Esc、R、Backspace 不动（既有模块）。**焦点环/aria/焦点管理三件套**——全局 `:focus-visible`（2px primary offset 2）逐元素核查补缺：`.treemap-wrap:focus-visible`（显式声明）、`.strip-block:focus-visible`（inset 环——`.strip-bar` overflow:hidden 裁外环；条带块 tabindex=0+role=button+Enter/Space 键盘可达）；既有 button/input/行/面板条目规则齐全未动；**treemap 容器 aria-label 摘要**（renderTreemap/doLiveRefresh 更新：「当前目录 X，共 N 项，最大子项 name（size_human）」——数据自 browse 载荷经本模块就地传入）；路由切换焦点管理核对（router.js [data-page-title] tabindex=-1+focus 既有）；toast aria-live 既有（核对归属）。**reduced-motion 降级总表逐行复核**（L0×5/L1×5/L2×11/L3×9 + §3.5 第 217 行总表：循环类全局 .01ms、入场类直显、转场类直切、L2-4 不播、toast/shake 功能性保留 ≤120ms——代码逐行核对通过；键盘导航属功能性不降级；焦点环静态不依赖动画）。**纯键盘全旅程录屏留档**（u41 ⑨：浏览|扫描|对比|设置四段全键盘 + 录屏 webm + 焦点环亮/暗截图——https://gallery 见 %TEMP%\u41_acc_shots）。⏱️ 冒烟 **A20 实装**（键盘矩阵语义：容器聚焦/tabindex、焦点块初始化+APP_STATE.treemap.focusIdx 单一来源、静态描边像素采样、方向键移动+边界静止+回退、Enter 恰 1 次 browse 且 path=焦点块且双击窗互斥、/ 聚焦+双守卫、g c/g s+守卫全系（isComposing/超时/输入框/弹窗栈）、keys.js 直断言、Backspace 守卫共享——smoke 脚手架无 style.css 画布失控：A20 前置显式约束容器尺寸）。**验收**：smoke v2 **21/21**（A20 新接入）；u41_acc_probe（新建）**46/46**（桩态 23——最近邻数学/运行时移动/边界/焦点块像素/容器焦点环 computed 2px primary offset 2/Enter 恰 1 次+path/双击窗互斥/Backspace 输入框守卫+goUp 恰 1 次// 聚焦+三守卫/g c g s+四守卫/console 0＋reduced 7——键盘导航功能性保留/描边静态/CSSOM 规则常驻/Enter 功能保留＋双档零滚动 2——1366×768/1920×1080 焦点态＋真机全旅程 16——扫描发起态+收束/Tab 序列/方向键/Enter·Backspace 请求发起态/筛选/g c 对比页装配/g s/设置+Esc/主题暗色/焦点可见/console 0/录屏留档）；回归 u31 **38/38**（⑧ / 语义适配 focus-filter）、u34 **44/44**、u35 **50/50**、u24 **34/34**、u33 **31/31**、u32 **45/46**（⚠️ 基线核对：⑧「50 次循环 DOM 节点数±5」before=466/after=459 在 **ui2.0 基线 worktree 逐字节复现**——预存漂移非本轮回归，挂账注记）；unittest **266 OK**（P13 首轮复现一次、复跑即绿；零 py 改动）；node **25/25**（+5 nearestFocusIndex 用例）；hex **0**（style.css 新增规则全部 token 引用）。⚠️ 偏差注记：①/ 聚焦筛选框（见上，源码为准）；②真机全旅程顺序=浏览→扫描→对比→设置（⚠️ 本机 Everything SDK 直扫分钟级且查询串行化（G8 升级）：扫描收束后 SDK 长忙、browse 排队/409 数百秒——「扫描→浏览」序浏览步不可达，多轮实测后按用户裁决：浏览步先行 + 真机下钻/返回降级为「请求发起态+录屏」断言（Enter/Backspace 请求恰 1 次 path 正确——完成态语义由桩态 ③⑥b/A20/node 确定性验证）；③reduced-motion 下 Chromium 以系统焦点环替代表格化 author outline（div[tabindex] 均受影响——可见性保留（2.67px 系统环），正常模式 2px primary 由 ②b 实测；CSSOM 作者规则常驻；已注记）；④u32 ⑧ 基线预存；⑤G1/G2/G5（F06 last-browse/F07 历史下拉/N12 折叠）维持挂账——U4.x 收口项，本轮未裁决（顺手续做与批次四专项无关，避免铺开）。
> - **U3.5：弹窗族与引导收尾（批次三闸门项；实现 + 验收完成）。** `components/settings.js`**主题三态（F03/N03）**：设置弹窗新增「主题：亮/暗/跟随系统」分段单选（`#setting-theme` 组 + `#setting-theme-light|dark|system`，name=setting-theme——既有设置弹窗 id 全不变）＋**单一来源** `theme.js` 三值偏好（`pds_theme_v1`=light|dark|system；**缺失/非法 key=system**）经 `themePref/resolvedTheme` 解析（system→matchMedia；与 index.html head 防闪烁脚本同口径——脚本对非 light/dark 值一律 matchMedia=显式 system 与缺失 key 同路径）；`setThemePref(pref,ev)`（三态入口：持久化+生效主题+VT 圆形扩散 450ms（L0-1 复用）+matchMedia 监听）与 `switchTheme(next,ev)`（**签名/键值语义不变——A1 零适配**；非法入参=翻转当前生效主题）；顶栏=亮↔暗显式翻转（写入显式偏好退出 system）；设置单选**即生效**（与「保存设置」解耦；VT 原点=所选选项中心——change 事件无坐标）；**同源联动**（改一处另一处反映）；matchMedia change 监听仅 pref=system 激活且**模块加载即挂**（重开/刷新后 OS 变化仍实时跟随）；saveSettings POST theme 由固定 "light" 改报 `resolvedTheme()`（后端白名单 light|dark 校验不变，零 py 改动——偏差注记③）｜**危险区 L2-10**：wipe-panel 红描边脉动 2.4s（`.wipe-panel::before` 边框环 `wipe-pulse`，**仅 opacity**；`--dur-breathe` 新增=**badge-breathe 2.4s 同值 token 化**（.badge 规则同步改用））＋输入匹配「确认清空」→**3s 倒计时解锁**（`--dur-wipe-countdown` 新增；倒计时期间按钮**禁用+文案「确认清空（Ns）」**；失配→取消武装重计；重开复位；倒计时=安全语义 reduced **不降级**；wipeData 失败复位=原语义立即可重试）｜**Toast L2-6 全参数**（**驱动在 toast.js/WAAPI、样式在 style.css**——同 compare.js L3-6 模式，参数可经 getAnimations 在 smoke 断言）：滑入 320ms spring（`--dur-3`/`--ease-spring`；reduced→`--dur-1` 120ms）＋时间线（自动消失进度 scaleX 1→0；时长=TTL 4000/6500ms 动态；类型色 `--success/--danger/--warning/--primary`）＋成功描边 300ms（`.toast-stroke` 层 opacity 呈现，`--dur-toast-stroke`）＋错误脉动 2 次（`--dur-toast-pulse` 600ms=2×300ms、delay 320ms 至滑入结束、scale(1.02)+opacity）＋hover 暂停（计时器与时间线 WAAPI 同步 pause/play）＋aria-live=polite（容器幂等）；reduced：装饰性脉动/描边跳过+时间线暂停+CSS 层隐藏（功能保留=toast 仍出现/自动消失）；既有调用方签名不变 `toast(msg,kind[,timeoutMs])`；`.leaving` 类名兼容保留｜**批次三闸门核对单**：`docs/UI2.0_批次三闸门核对单.md` **新建入库**（F01–F24/N01–N13 全表逐项：22✅ / 3◐ 功能等价降级 / **挂账 6 项 G1–G6**——G1 F06 启动恢复 pds_last_browse_v1 未接、G2 F07 历史下拉形态（chips+面板数据源功能等价）、G3 F22「已选 N 项」未接（预存 U4.x）、G4 F23 键盘矩阵→U4.1 专项、G5 N12 面包屑>6 层折叠（home 语义等价）、G6 L3-5 sparkline 降级（后端补字段后启用）；环境性 G8（SDK 直扫分钟级）/G9（真机清空走查实测**成功路径**——本机数据目录可写，真实快照被清并以真扫+保存恢复 1 会话））｜tokens.css **+4**（`--dur-breathe/--dur-wipe-countdown/--dur-toast-stroke/--dur-toast-pulse`；无 hex 未动既有值）；style.css 新「U3.5 弹窗族与引导收尾」分区 **hex 0**（主题分段单选/wipe-panel 脉动/toast 布局与类型色；动画仅 transform/opacity）；index.html：head 防闪烁注释三态化+设置弹窗主题字段+wipe-panel 类｜**测试与验收**：smoke v2=**20/20**（**A19 新接入**：主题三态语义/危险区倒计时/toast 时间线 4000ms·scaleX/成功描边 300ms/错误脉动 600ms delay 320ms/hover 暂停/aria-live；**A1 零适配**）；`u35_acc_probe.mjs`**新建**=**60/60**（桩态 43+reduced 7+真机 10：主题三态 13 项（缺失 key=system/显式 system/单选即生效+持久化/顶栏翻转同源/OS 实时跟随（emulateMedia 切暗亮）/刷新后 head 三态解析一致/listener 重挂）/危险区 11（wipe-pulse 2.4s/倒计时禁用+秒数/3s 解锁/失配重计/重开复位/执行链 confirm 正确+弹窗关闭+联动）/toast 11（全参数+类型色 token 对照）/reduced 5/弹窗栈回归 2（Esc 逆序=红线#9/A7 不破）/双档零滚动+截图 3/console 0+真机 10（设置三态切换/清空流程真实走查：输入→倒计时→执行→**目录重建真实成功**/console 0/双档零滚动））；**零 py 改动**；回归 u31 **38/38**·u24 **34/34**·u34 **48/48**（首轮真实页 1 项失败=u35 真机清空走查后的环境态变化（快照被清）非代码回归——G9 已记，恢复后复跑全绿）；unittest=**266 OK**（P13 复现 3 次复跑即绿；snapshots.py/compare.py/scan.py/app.py 禁碰保持）；node=**20/20**；hex=**0**；双档零滚动 1366×768/1920×1080（设置弹窗/清空弹窗/toast 态）+视觉目检 11 张截图（亮/暗设置弹窗（三态分段控件选中态高亮）/清空弹窗（红描边脉动环+倒计时（3s）禁用态）/success toast（绿描边+绿时间线）/error toast（红边+红时间线）/reduced 静态）｜⚠️偏差注记：①§3.2 `APP_STATE.theme` 语义扩展为**偏好**三值（light|dark|system；data-theme=解析生效值——state.theme=偏好、currentTheme()=生效，双口径）；②smoke 页无 style.css——CSS 层动画参数（wipe-pulse 2.4s 等）断言归 u35 探针（真实页），smoke 断言 WAAPI 参数（与 A18 手法一致）；③saveSettings POST theme 固定 "light"→`resolvedTheme()`（行为如实化；后端契约/白名单不变）；④A19 追加于 A18 之后运行（A18 finally 清场后无干扰）；⑤真机清空走查破坏环境快照数据（G9：验收流程本身要求，重建已恢复）。
> - **U3.4：对比工作台页（#/compare；实现 + 验收完成）。** `pages/compare.js` 占位页全量填充——布局按 §3.3/§3.5：页头 64px 行式（标题「历史对比」+ 基线 datalist（迁移 rebuildBaselineSuggest，全量快照路径回灌）+ **目标只读** + 开始对比）→ 摘要 3 卡 96px（总变化=report.delta_total / 最大增长=Top100 中 max(delta>0) / 可释放=Top100 中 Σ|delta<0|，count-up L1-4 600ms）→ 红绿发散图 240px（**L3-6 全参数**：中轴基线、增长条**向左红** --up、缩减条**向右绿** --down、条宽=|Δ|/max|Δ|×半轨（≤50%，初版 100% 满轨溢轴目检发现已修）、中轴生长 500ms（tokens.css **+1 `--dur-diverge:500ms`**；仅 transform scaleX——动画只动 transform/opacity）、增速徽标 pop-in scale .8→1（--dur-3 320ms + --ease-spring；仅 transform/opacity）、▲/▼± 冗余符号、持平行=中轴中性标记）→ 表格 flex:1 面板内滚（列 变化=▲/▼±+signedBytes+tabular-nums / 增速 / 路径+新增·已删除标签 / **F19 操作=定位 open-path + 复制路径 act-copy-cmp 行为保留**；L1-2 行 stagger 前 12 行 --dur-stagger-row；truncated/legacy_count/W2.11 数据时间状态行保留）｜⚠️**数据源口径核对结论（本项核心）**：app.py api_compare 执行核对——「当前」侧数据 = **fullscan.result(root) 缓存 or SDK 直扫**（非快照文件）→ **「目标=同盘符最新快照」为展示口径（只读标识性文案）**：对比目标实为当前磁盘状态；页面 label title 已注明该口径，**零后端改动**（偏差注记①）｜流程（定稿 6.4）：**三入口预填**——趋势卡（trio baseline/root/target + **结果共享**：snapshots.js fetchTrendCompare 缓存整份 report、prefillAndGoCompare 写入 APP_STATE.compare.result，落地即渲染不回源）/ 迷你条目（trio 预填，root/target 本地推导防 snapshot-mini⇄snapshots 环）/ 直达（默认=当前盘最新快照；root 消费 §6.4 附加键）→ 自动填基线 → 目标=该盘最新快照（只读）→ **骨架屏**（L1-5 复用 skel-* shimmer）→ 摘要/发散图/表格；**结果缓存 APP_STATE.compare.result**（{root,baseline,report,at}）：路由往返回灌不重发（smoke A18/A17、u33 ③b 计数口径保持）+ markNavDot 仅真实完成挂点（回灌不挂圆点防跨路由残留）｜空态（定稿 6.5「选择一份基线快照，开始对比两个时间点的空间变化。」；无快照不自动对比）｜**旧工作台「历史对比」卡迁整页并移除**（F18：ids #compare-baseline/#btn-compare/#compare-result/#compare-status/#compare-body 随迁页面，主页仅留「最近对比」迷你卡）；compareSnapshots 保持 **DOM 无关**（页面未挂载仅记账/圆点——u31 ⑤② 跨页触发路径）；W2.4 409 语义保留 + **锁释放自动重试（上限 3 防轮询风暴）+ 状态行如实文案**；W2.13 异机确认保留；palette「开始对比」改指 #/compare 并预填默认（清 baseline/result 强制新一轮）；scan.js renderFullscanState `$("btn-compare")` 与 settings.js wipeData `$("compare-result")` 空守卫/迁移（清空→resetCompareData 全量复位：结果/迷你摘要/对比页回空态）；**顺手修复**：workspace.js browsePath 失败分支 `$("browse-root")` 空守卫——U3.4 启动期对比自动执行与 init 链自动浏览竞争 SDK 锁时 409，子页面（#/compare 无输入框）→「Cannot set properties of null」TypeError 真机复现后修复（空守卫纪律补漏，见偏差注记②）｜style.css 新「U3.4 对比工作台页」分区（hex **0**；移除旧对比卡死规则 .compare-chart/.change-row/.change-bar/.compare-summary/.stat*）；tokens.css +1（--dur-diverge:500ms，增补规则无 hex 未动既有值）｜smoke **A18 接入 19/19**（预填链路/自动执行+骨架/摘要 count-up 记账/发散图语义+500ms+pop-in/表格 F19/空态/迷你卡 lastSummary/往返不重发）；unittest **266 OK**（P13 复现一次复跑即绿，零 py 改动）；node **20/20**；hex **0**｜`scripts/dev/u34_acc_probe.mjs` 新建 = **48/48**（桩态 40：①装配（页头三件套/只读/datalist 3/副行盘符/挂载自动预填）②摘要（3 卡/记账 2000·2000·1000/终值精确/▲▲▼）③发散图（4 行/中轴/增长锚左红/缩减锚右绿/符号/240px）④L3-6 参数（骨架屏/500ms scaleX/徽标 320ms pop-in/缓动 token/自动重试）⑤表格（4 行/▲▼±/新增标签/F19 open-path 实调/复制 toast/面板内滚）⑥空态（不自动对比/定稿 6.5/结果隐藏）⑦迷你条目 trio ⑧趋势卡 trio+结果共享不重发 ⑨50 次对比无泄漏 ⑩双档零滚动+截图 + reduced 4（直显终值/无动画/表格照常）+ 真机 4（真实会话装配+挂载自动对比发起/双档零滚动/console 0——⚠️ 偏差注记③：本环境 Everything SDK 直扫分钟级（单目录 19 文件实测 >300s 超时；U2.2「IPC 抖动 1-8s」在本机为分钟级，预存环境性非前端缺陷）→ 真机「对比完成」窗口内不可达，完整对比渲染链由桩态确定性验证（真实 tokens/style.css/渲染代码），真机阶段如实断言「发起态」）；回归 u33 **31/31** ✓、u31 **38/38** ✓（⑤② 触发改经 compareSnapshots 跨页路径——工作台对比卡已迁子页面）、u24 **34/34 + 扫描态 3/3** ✓（最近对比摘要经 #/compare 真实路径；真实扫描态在活扫描窗口复验）、u25 **46/46** ✓；视觉多模态：亮 1366/暗 1366/亮 1920/空态/窄屏 800×700 截图目检合格（发散图几何/摘要卡/表格/空态；发现并修复条宽溢轴）｜⚠️挂账：L3-5 sparkline 仍为降级差值卡（待后端补逐次总量字段）；「目标=最新快照」展示口径偏差注记①；真机对比完成依赖后台全量结果缓存（本环境全量扫描同样分钟级，验收窗口内不可达——偏差注记③，非前端缺陷，正常桌面环境不受影响）。
...
> - **U3.3：快照管理页（#/snapshots；实现 + 验收完成；可与 U3.4 并行）。** `pages/snapshots.js` 占位页填充：布局按 §3.3/§3.5——页头 64px 行式（标题 + 创建快照 F15 + 撤销最近保存 F16）→ 趋势区 128px 趋势卡×2（N07）→ 会话分组列表 flex:1 面板内滚（F17）；⚠️**字段核对结论（本项核心）**：`/api/snapshots` 会话数据（app.py api_snapshots + session.py save_session 双核对）**无「逐次总量」字段**（session 载荷仅 session_id/auto/machine_guid/roots/ledger_backup/created_at；roots 条目仅 snapshot 名/路径/skipped/skip_reason/notice）→ **L3-5 sparkline 按手册 U3.3 预案降级为「两快照对比差值卡」**（无折线，保留 ▲/▼ 与百分比；**零后端改动**，smoke/探针断言 0 svg 证实无折线；L3-5 留待后端补总量字段后启用）｜**N07 基线口径展开**（偏差注记①）：较昨日=同盘符快照 0<Δt≤24h 最近一份；较上周=(24h,7d] 最近一份（**排除 ≤24h**——否则与较昨日同基线、两卡信息重复；按「周变化」语义取窗口内最近一份；参考图两卡不同值佐证）；目标=该盘最新快照；无合适基线 → 定稿 6.5「暂无对比基线」；多盘均合格时取「最新目标」首个盘（时间倒序确定性；真机实测 08-29 双盘同会话并列时 C:\ 先于 D:\——按根名 tie-break，已记）；Δ=复用 `/api/compare`（{root,baseline}，各卡恰 1 次；trendCache+trendInflight 双防重发；**路由往返从缓存回灌不重发**——smoke ③/探针 ③b 计数断言）｜点击卡 → APP_STATE.compare 预填（baseline/root/target，§6.4 跨页形态，U3.4 消费；⚠️ §3.2 compare 形状之外**附加 root 键**（additive，U3.4「目标=同盘符最新快照」需根））→ #/compare｜页面接线：页头动作经 main.js **setSnapshotsActions 注入**（create=saveSnapshot 复用保存流程[F15] / undo=undoLastSave 确认弹窗流[F16]——**防 scan↔snapshots 环**：snapshots.js 零 scan.js 依赖，模式同 palette 注入；scan.js 新增导出 `getLastScanStatus`/`isSaveAvailable`）；「创建快照」可用性=`isSaveAvailable()`（镜像扫描卡保存按钮派生：完成+save_ready / 中止+部分根；N06「无全量数据时置灰+提示」；pds:scan 模块级单订阅同步）；**列表迁移 F17**：renderSnapshotList 语义迁移（会话分组/auto·manual 标签/逐盘「对比此快照」=预填+跳 #/compare[§6.4 跨页形态]/跳过原因 **tooltip=红线 #7 SKIP_REASON_TEXT**（文案可见 + title 悬停 + aria-label））；三处空态（定稿 6.5：列表「还没有快照…」/趋势卡×2「暂无对比基线」/迷你卡空态**新增「打开快照管理」单按钮**——N06 要求补全）；**工作台迷你卡收敛**：撤销/刷新/「全部会话」折叠区随子页面接线移除（折叠区含 #snapshot-list 迁子页面；#btn-undo-save 迁页头同位）→ scan.js bindScan/compare.js bindCompare 改**空守卫**（原无守卫绑定会在工作台挂载 TypeError——U2.1 空守卫纪律补漏；compare.js 的 act-cmp-snap 委托仅留兼容守卫，实际语义落在快照页）；settings.js wipeData 改 applySnapshotsView() 空缓存回灌（清空后列表+趋势卡+迷你卡+基线建议全量复位，原仅列表——趋势卡接入后的必需联动）｜CSS：style.css 新「U3.3 快照管理页」分区（.page-head-row/**.trend-row 128px**/.trend-card 差值卡（▲▼+百分比+基线→最新副行；hover/focus-visible）/snapshots-list-wrap 内滚/**.session-roots 补样式**（原未样式化——默认 ul 圆点+缩进，现 list-style:none + 行布局 + code 芯片）/skip-reason cursor:help；**hex 0**；无新动画（仅 border/shadow 过渡读 --dur-1；动画仅 transform/opacity 红线保持）；死规则清理：.snapshot-mini-tools/.snapshot-mini-list 删除、.snapshot-list 去 320px 限高（内滚移交页面）；tokens **零新增**（复用 --fs-*/--radius-*/--danger/--success/--muted 等）｜测试：smoke v2 = **18/18**（**A17 新接入**：页头装配/创建置灰/单快照无基线×2/3 会话趋势卡基线 ≤24h 与 (24h,7d] 且 ▲▼+百分比+0 svg/两卡恰 2 次 compare/点击预填 baseline·root·target/返回回灌不重发/逐盘对比预填/跳过 tooltip/撤销确认流；A8 只增不删——U3.2 A16 同理）；unittest = **266 OK**（⚠️ **P13 首轮复现 1 fail**（test_budget 并发竞态）、复跑即绿——本项**零 py 改动**，未触碰 snapshots.py/compare.py/scan.py）；node = **20/20**；hex = **0**；u33 验收探针（新建 scripts/dev/u33_acc_probe.mjs）**35/35** = 桩态 31（①装配+置灰/②基线/降级 0 svg/compare 恰 2/③预填+回灌/④逐盘/⑤撤销确认流/⑥创建启用+POST save/⑦空态三处/⑧50 次路由往返节点无增长+console 0/⑨双档零滚动+工作台回归/⑩亮暗·1920·800 截图）+ 真实页 4（真实 2 会话装配/双档零滚动/console 0——browse/compare 并发 409 资源日志按 U3.2 注记 4 口径过滤）；回归：u21 ✓（转场 237ms≤360、errors 0、快照数据回灌=1——探针快照断言已适配迷你条目）、u24 **34/34**（折叠区/空态断言按 U3.3 迁移适配）、u25 **46/46**、u31 **38/38**｜⚠️偏差注记：①「较上周」窗口口径 (24h,7d] 为 N07/D7 展开（字面 ≤7d「最近一份」与较昨日窗口重叠——两卡基线必同、信息重复；取「周变化」窗口内最近一份保持双卡差异语义）；②会话列表「对比此快照」行为由「工作台内联直比」改为「预填+跳 #/compare」（§6.4 跨页形态；#/compare 仍占位至 U3.4——同 N06 迷你卡先例，U3.4 落地后自动闭环）；③default stub 单会话样本→趋势卡空态；A17 trend 样本带 C:\ 跳过盘（day_budget_exceeded）验证红线 #7；④trendCache 为模块级持久缓存（键=槽:根:基线）——同基线结果不重复算；wipe/清空后 applySnapshotsView 空渲染但缓存条目残留（路径唯一无碰撞，不清理）；⑤真实页趋势卡在扫描在途/慢 SDK 直扫时停留在「正在计算对比…」——后端 compare 异动性，非前端缺陷；⑥工作台迷你卡空态原为纯文案（U2.4 缺 N06 单按钮要求，本项补全）｜⚠️挂账：无新增（F22 仍 U4.x；P13 沿用）。
...
web/static/
  ...
  js/app.js               【已删】U4.3 物理删除（U2.0 起为空壳注释过渡；index.html 仅引 main.js，零引用）
web/templates/index.html  【重构】App Shell 骨架
tests/web/smoke.html      【改造】仅 v2 注册表（A0–A20；v1/legacy 于 U2.5 退役删除）
```
...
| A16 | 扫描控制卡 | 停止接口特性探测（404 隐藏停止按钮）+ 扫描中/中止态装备与文案 + chips 三态 + 无 K7 提示 | U3.2 |
| A17 | 快照管理页 | 页头装配（创建/撤销）+ 趋势卡×2 降级差值卡（▲/▼+百分比+无折线）+ 基线选取 + 点击预填跳转 + 跳过原因 tooltip（红线#7）+ 逐盘对比预填 | U3.3 |
| A18 | 对比工作台页 | 页头装配（基线 datalist+目标只读+开始对比）+ 三入口预填 + 自动执行与骨架屏 + 摘要 3 卡 count-up + 红绿发散图 L3-6 + 表格行 F19 + 空态（定稿 6.5） | U3.4 |
| A19 | 弹窗族与引导收尾 | 主题三态（设置弹窗亮/暗/跟随系统 + 顶栏同源联动 + system=matchMedia 解析与持久化）+ 危险区 L2-10（规则位+输入匹配→3s 倒计时禁用→解锁·失配重计）+ Toast L2-6（320ms spring+时间线+成功描边+错误脉动+hover 暂停+aria-live） | U3.5 |
| A20 | 键盘矩阵（U4.1） | treemap 容器聚焦（tabindex=0）+ 焦点块初始化（focusIdx 单一来源）+ 静态描边像素可见 + 方向键最近邻移动/边界静止/回退 + Enter 恰 1 次 browse 且 path=焦点块（双击窗互斥）+ `/` 聚焦筛选框（双守卫）+ g c/g s 连按跳页（isComposing/超时/输入框/弹窗栈守卫）+ keys.js 共享守卫直断言 | U4.1 |
